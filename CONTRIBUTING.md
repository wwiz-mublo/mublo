...
```bash
composer install
composer check   # 게이트 전체 + 테스트 (CI 와 동일)

# composer test                 # 테스트만
# composer check-security       # 운영 의존성 취약점
# composer check-di             # Service/Controller 의 직접 인스턴스 생성
# composer check-extension-api  # 확장이 안정 API 만 쓰는지
# composer check-strict-types   # 런타임 파일의 declare(strict_types=1)
# composer analyse              # PHPStan level 0 (전 범위)
# composer analyse-strict       # PHPStan level 3 (코어·Board·Shop·플러그인)
```

런타임 PHP 파일은 `declare(strict_types=1)` 을 선언합니다. 새 파일이 빠지면
`check-strict-types` 가 실패합니다. 뷰 템플릿과 테스트는 대상이 아닙니다.
...
2. 기능 브랜치에서 작업합니다
3. 전체 CI 게이트를 통과시킵니다 (`composer check`)
4. PR을 생성합니다
