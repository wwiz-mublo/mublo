...
- 쇼핑몰: 자동 생성되는 상품코드(`item_code`)가 충돌해 상품 등록이 실패하던 문제를 고쳤습니다. 코드는 도메인 무관 전역 유니크인데 4자리 난수로 뽑고 재시도가 없어서, 생일 문제로 하루 118건 정도면 충돌 확률이 50%에 이르렀습니다. 충돌하면 "상품 생성에 실패했습니다"라는 원인 불명 메시지만 남았습니다. 이제 그날의 마지막 번호 다음을 쓰고, 이미 있으면 다음 번호로 넘어갑니다. 기존 코드는 그대로 두며 형식(`G-YYYYMMDD-NNNN`)도 같습니다
...
- **쇼핑몰: `쇼핑몰 → 등급별 혜택` 관리자 화면이 사라집니다.** 이 화면이 저장하던 등급 가격정책(할인율·적립율·무료배송·자동쿠폰)은 어떤 계산에도 연결된 적이 없어, 값을 넣어도 아무 일도 일어나지 않았습니다. 등급별 할인·적립의 저장소는 위 Added 의 JSON 설정(쇼핑몰 설정 화면의 "레벨별 할인 값"과 상품별 등급 설정)으로 확정했으므로, 같은 목적의 화면 두 벌 중 쓰이지 않던 쪽을 걷었습니다. **무료배송·자동쿠폰은 이 화면에서도 동작한 적이 없으며, 이번 변경으로 새로 빠지는 기능은 없습니다.** `shop_level_pricing` 테이블은 남겨 둡니다 — 삭제는 되돌릴 수 없으므로 정리 시점은 따로 판단합니다
