...
### Fixed
- Core: 이벤트 디스패처가 부모 클래스·인터페이스 이름으로 등록한 리스너를 실행하지 않던 문제. 세분화된 서브클래스로 발행되는 이벤트를 상위 이름으로 구독하면 조용히 무시됐다
- Plugin(MemberPoint): 위 문제로 지급되지 않던 가입 축하 포인트 복구
- Package(Shop): 위 문제로 발행되지 않던 LEVEL 트리거 쿠폰 자동발행 복구
