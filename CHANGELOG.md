...
- 에디터: 문서 가져오기의 DOCX·XLSX·PDF, 스마트 붙여넣기의 OG 카드는 서버 처리기를 붙였을 때만 나타납니다. 이번 릴리즈에는 그 처리기가 없으므로 파일 선택 목록과 붙여넣기 선택지에 나오지 않습니다. 문서 내보내기의 PDF 도 페이지에 `html2pdf.js` 를 실은 경우에만 열립니다 — 라이브러리를 번들하지 않습니다
