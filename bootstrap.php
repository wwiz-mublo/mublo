...
// PHP 에러와 error_log() 호출의 목적지를 storage/logs 안으로 고정한다.
//
// 코드에는 두 갈래가 있다. Logger 는 storage/logs/D{도메인}/{채널}/{날짜}.log 로
// 쓰고, error_log() 는 Logger 를 쓸 수 없거나(부팅 중·DI 미주입) Logger 자체가
// 실패했을 때의 폴백이다. 그 분리는 의도된 것이다.
//
// 문제는 폴백의 목적지를 지정하지 않았다는 점이었다. php.ini 기본값으로 떨어지면
// 공유호스팅에서는 운영자가 볼 수 없는 곳이거나 아예 사라진다. 안전망은 있는데
// 바닥이 뚫려 있던 셈이다.
//
// 한곳에 모으면 storage/logs 를 지우는 일일 정리(DailyStorageCleanup)에도 함께
// 포함된다. 디렉터리가 없거나 쓸 수 없으면 PHP 가 알아서 기본 목적지로 돌아가므로
// 지금보다 나빠지지는 않는다.
if (is_dir(MUBLO_STORAGE_PATH . '/logs')) {
    ini_set('error_log', MUBLO_STORAGE_PATH . '/logs/php-error.log');
}
