 ...
 */
$pending = $pending ?? [];
$pageTitle = $pageTitle ?? ($title ?? '포인트 플러그인 설치');
?>
...
<div class="card">
    <div class="card-body text-center py-5">
        <i class="bi bi-coin text-secondary" style="font-size:3rem;"></i>
        <h4 class="mt-3">포인트 플러그인 설치가 필요합니다</h4>
        <p class="text-muted mb-4">데이터베이스 테이블을 생성해야 포인트 기능을 사용할 수 있습니다.</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        ...
        MubloRequest.requestJson('/admin/member-point/install', {}, { loading: true })
            .then(function () {
                location.reload();
            })
            .catch(function (err) {
