...
<li><strong>가격:</strong> 전역 할인·적립, 등급별 할인·적립, 상품별 오버라이드, 쿠폰이 겹친 최종 금액을 확인합니다.</li>
...
<li><strong>고객·마케팅:</strong> 할인쿠폰, 구매후기, 상품문의, 찜한상품, 기획전</li>
</ul>
<h3>운영자가 기억할 원칙</h3>
<ul>
...
<p>구매후기 적립금은 전 등급 공통값입니다.</p>
<p>등급별 값을 두는 화면은 따로 없습니다. 할인·적립 유형에서 <strong>레벨별</strong>을 고르면 이 화면에서 바로 등급마다 값을 지정하며, 상품 등록 화면에서 상품별로 덮어쓸 수 있습니다.<br>전역·등급·상품 세 층이 겹치므로 전체 정책을 먼저 설계한 뒤 혜택이 의도치 않게 중복되지 않는지 확인하십시오.</p>
<h3>검증 예시</h3>
...
<h3>상품정보제공고시</h3>
<p>프론트 상품 상세에 상품정보제공고시를 표시하려면 상품 등록·수정 화면에서 품목을 선택하고 해당 양식의 정보를 입력합니다.</p>
<ul>
<li>품목을 선택하지 않으면 상품정보제공고시 탭이 표시되지 않습니다.</li>
<li>모든 항목은 선택 입력이며 값이 있는 항목만 프론트에 표시됩니다.</li>
<li>정보가 상품 상세설명에 제공되어 있다면 항목별 또는 빈 항목 일괄 기능으로 <strong>상품 상세설명 참조</strong>를 입력할 수 있습니다.</li>
<li>고시 양식이 개정되어도 기존 상품은 작성 당시 양식과 입력값을 유지합니다.</li>
</ul>
<p>상세설명이 이미지로만 구성된 경우 정보 접근을 위한 대체 텍스트도 제공하십시오.<br>입력 내용과 법령상 정보 제공 의무의 충족 여부는 운영자가 확인해야 합니다.</p>
...
HTML,
        ...
        [
            ...
            'content' => <<<'HTML'
<h3>등급별 할인·적립</h3>
<p>등급별 혜택을 위한 전용 화면은 없습니다. <strong>쇼핑몰 설정</strong>(<code>/admin/shop/config</code>)의 할인·적립 유형에서 <strong>레벨별</strong>을 고르면 등급마다 정률·정액 값을 지정할 수 있고, 상품 등록 화면에서 상품별로 덮어쓸 수 있습니다. 설정 방법은 <strong>쇼핑몰 설정: 할인·적립·등급</strong>을 참고하십시오.</p>
<ul>
<li>비회원이거나 해당 등급에 값이 없으면 등급 할인·적립은 적용되지 않습니다.</li>
<li>포인트 사용 한도의 등급별 값은 같은 설정 화면의 결제 영역에 있습니다.</li>
</ul>
<blockquote><p>등급별 무료배송과 등급 자동 쿠폰 발행은 제공하지 않습니다. 값을 입력받는 화면이 있었으나 어떤 계산에도 연결되지 않아 화면과 함께 제거됐습니다. 등급 대상 쿠폰이 필요하면 쿠폰의 자동 발행 조건을 사용하십시오.</p></blockquote>
<h3>쿠폰 정책</h3>
<ul>
