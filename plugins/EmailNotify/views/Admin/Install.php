 ...
 */
$pending = $pending ?? [];
$pageTitle = $pageTitle ?? ($title ?? '이메일 알림 플러그인 설치');
?>
...
<div class="card">
    <div class="card-body text-center py-5">
        <i class="bi bi-envelope-paper-heart text-secondary" style="font-size:3rem;"></i>
        <h4 class="mt-3">이메일 알림 플러그인 설치가 필요합니다</h4>
        <p class="text-muted mb-4">데이터베이스 테이블을 생성해야 이메일 알림 기능을 사용할 수 있습니다.</p>
...
<script>
// 어드민 일관성: 머블로 알럿 모달 비활성(아래 .catch가 alert + 버튼 복구 담당).
MubloRequest.configure({ errorHandler: function () {} });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        if (!confirm('이메일 알림 플러그인을 설치하시겠습니까?\n데이터베이스 테이블이 생성/변경됩니다.')) {
            return;
        }
        ...
        MubloRequest.requestJson('/admin/email-notify/install', {}, { loading: true })
            .then(function () {
                location.reload();
            })
            .catch(function (err) {
