...
class SnsLoginServiceTest extends TestCase
{
    public function testAutoRegisterUsesGeneratedNicknameAndRecordsOriginDomain(): void
    {
        ...
        $memberRepository->method('existsByNickname')->with(7, $nickname)->willReturn(false);
        $memberRepository->method('existsByOriginAndNickname')->with(7, $nickname)->willReturn(false);
        $memberRepository->expects($this->once())
            ->method('create')
            ->willReturnCallback(function (array $data) use (&$capturedMember, $database): int {
        ...
        $this->assertSame(7, $capturedMember['domain_id']);
        $this->assertSame(7, $capturedMember['origin_domain_id']);
        $this->assertSame('group-a', $capturedMember['domain_group']);
    ...
    public function testAutoRegisterRetriesAfterDatabaseUniqueKeyCollision(): void
    {
        ...
        $memberRepository->method('existsByNickname')->willReturn(false);
        $memberRepository->method('existsByOriginAndNickname')->willReturn(false);
        $memberRepository->expects($this->exactly(2))
            ->method('create')
            ->willReturnCallback(function (array $data) use (&$createCalls): int {
                $createCalls++;
                if ($createCalls === 1) {
    ...
    public function testAutoRegisterStopsAfterNicknameAttemptsAreExhausted(): void
    {
        ...
        $memberRepository->method('existsByNickname')->willReturn(true);
        $memberRepository->expects($this->never())->method('existsByOriginAndNickname');
    ...
    public function testConcurrentProviderLinkRollsBackNewMemberAndLogsIntoWinner(): void
    {
        ...
        $memberRepository->expects($this->once())->method('find')->with(777)->willReturn($member);
