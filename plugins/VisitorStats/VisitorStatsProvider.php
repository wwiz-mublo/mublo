...
class VisitorStatsProvider implements ExtensionProviderInterface, DataResettableInterface
{
    ...
    public function boot(DependencyContainer $container, Context $context): void
    {
        ...
        // 전환 기록 — 확장이 발행한 ConversionRecordedEvent 를 받아 쌓는다.
        // 발행자가 누구인지는 알지 못하고, 계약이 실어 온 sourceType 만 본다.
        $eventDispatcher->addSubscriber(new ConversionRecorderSubscriber(
            $container->get(ConversionEventRepository::class)
        ));
        ...
        BlockRegistry::registerContentType(
            ...
            options: [
                ...
                'capabilities' => BlockRegistry::capabilities(
                    skin: true, items: false, count: false, style: false, aos: true, customConfig: false,
                ),
