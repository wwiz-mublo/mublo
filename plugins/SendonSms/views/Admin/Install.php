 ...
 */
$pending = $pending ?? [];
$pageTitle = $pageTitle ?? ($title ?? '센드온 SMS 플러그인 설치');
?>
...
<div class="card">
    <div class="card-body text-center py-5">
        ...
        <h4 class="mt-3">센드온 SMS 플러그인 설치가 필요합니다</h4>
        <p class="text-muted mb-4">데이터베이스 테이블을 생성해야 센드온 SMS 기능을 사용할 수 있습니다.</p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btn.addEventListener('click', function () {
        if (!confirm('센드온 SMS 플러그인을 설치하시겠습니까?\n데이터베이스 테이블이 생성/변경됩니다.')) {
            return;
        }
        ...
        MubloRequest.requestJson('/admin/sendon-sms/install', {}, { loading: true })
            .then(function () {
                location.reload();
            })
            .catch(function (err) {
