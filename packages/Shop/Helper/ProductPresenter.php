...
class ProductPresenter
{
    ...
    public function toView(array $item, array $reviewStats = [], int $wishlistCount = 0, int $inquiryCount = 0): array
    {
        ...
        // 옵션/조합은 프론트 JS 공개 키만 전달한다.
        $transformed['options'] = $this->buildOptionList(
            is_array($item['options'] ?? null) ? $item['options'] : []
        );
        $transformed['combos'] = $this->buildComboList(
    ...
    private function buildOptionList(array $options): array
    {
    ...
    /** @return list<array<string, mixed>> */
    private function buildComboList(array $combos): array
    {
