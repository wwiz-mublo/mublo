...
<div class="page-container">
    <div class="page-title">
        <div class="page-title-text">
            <h3><?= htmlspecialchars($pageTitle ?? '교환 관리') ?></h3>
            <p>회수·검수·재출고를 주문 상태와 분리해 관리합니다.</p>
        </div>
    </div>

    <div class="card mb-3">
        <div class="card-header d-flex align-items-center">
            <div><strong>교환 상태 Action</strong><div class="small text-muted">주문 Action과 분리되며 알림·웹훅만 실행합니다.</div></div>
    ...
    <div class="card">
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                <thead><tr>
                    <th>번호</th><th>주문번호</th><th>상품</th><th>교환 옵션</th>
                    <th class="text-center">수량</th><th>상태</th><th>귀책</th><th>신청일</th>
                </tr></thead>
                <tbody>
                <?php if ($claims === []): ?>
                    <tr><td colspan="8" class="text-center text-muted py-5">교환 내역이 없습니다.</td></tr>
                <?php endif; ?>
                <?php foreach ($claims as $claim): ?>
                    ...
                    ?>
                    <tr>
                        <td><a href="/admin/shop/exchanges/<?= (int) $claim['return_id'] ?>">#<?= (int) $claim['return_id'] ?></a></td>
                        ...
                        <td><?= htmlspecialchars($claim['target_option_name'] ?? '동일 상품') ?></td>
                        <td class="text-center"><?= (int) ($claim['exchange_quantity'] ?? $claim['quantity'] ?? 0) ?></td>
                        <td><span class="badge bg-<?= $badge ?>-subtle text-<?= $badge ?>-emphasis border border-<?= $badge ?>-subtle"><?= htmlspecialchars($statusOptions[$status] ?? $status) ?></span></td>
                        <td><?= $resp ?></td>
        ...
        <nav class="mt-3"><ul class="pagination justify-content-center">
            <?php for ($page = 1; $page <= (int) $pagination['last_page']; $page++): ?>
                <li class="page-item <?= $page === (int) $pagination['current_page'] ? 'active' : '' ?>">
                    <a class="page-link" href="?<?= http_build_query(array_filter(['page' => $page, 'status' => $filters['status'] ?? '', 'keyword' => $filters['keyword'] ?? ''])) ?>"><?= $page ?></a>
                </li>
            <?php endfor; ?>
        </ul></nav>
    <?php endif; ?>
...
(function(){
 document.getElementById('saveClaimActions')?.addEventListener('click',()=>{
   ...
   MubloRequest.requestJson('/admin/shop/exchanges/actions',{actions:actions}).then(()=>location.reload());
 });
})();
</script>
