...
<form class="js-ship-form d-grid gap-2" data-action="<?= htmlspecialchars($action ?? '') ?>">
    <select name="company_id" class="form-select"><option value="">택배사 선택</option><?php foreach ($companies as $company): ?><option value="<?= (int) $company['company_id'] ?>"><?= htmlspecialchars($company['company_name'] ?? '') ?></option><?php endforeach; ?></select>
