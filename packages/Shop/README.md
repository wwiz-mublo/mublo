...
- **상품/카테고리** — 다중 이미지, 옵션(단일·조합·추가), 상품옵션 프리셋, 트리 카테고리, 상품정보 템플릿, 품목별 상품정보제공고시
...
- **운영/확장** — 관리자 대시보드·주문 리포트, 가격비교 피드, 상품/리뷰 블록, 통합 검색 연동, PII(주문·수령·배송지) AES-256-GCM 암호화
...
배송 완료 상품은 주문 상세에서 일부 수량을 교환 신청할 수 있습니다. 동일 상품·동일 옵션가 교환만 지원하며, 가격이 다른 상품이나 옵션은 반품 후 다시 주문해야 합니다.
...
승인 시 교환 재고를 예약하며 취소 또는 검수 거절 시 복구합니다. 회수품은 검수 결과가 판매 가능일 때만 재입고합니다. 교환 상태와 재고 처리는 주문 상태 처리와 분리되어 주문용 알림·포인트·재고 액션이 중복 실행되지 않습니다.
...
- `Event/`: 상품·카테고리·기획전·주문·결제·배송·교환 변경 이벤트
...
Shop은 상품, 카테고리, 기획전, 주문·주문상품, 결제, 배송, 교환의 변경 이벤트를 제공합니다. 모두 변경이 끝난 뒤 전달되는 확장용 이벤트이므로 접근 제어처럼 원 작업을 중단하거나, 결제·재고·포인트처럼 반드시 한 번 실행되어야 하는 처리에는 사용하지 않습니다.

필수 후처리는 Shop 서비스가 소유합니다. 외부 구독자는 동일한 재고·쿠폰·포인트 처리를 반복하지 않아야 하며, 주문 개인정보가 필요한 경우에도 이벤트의 내부 배열보다 공개 Snapshot API를 우선 사용합니다.
