...
class CategoryRepository extends BaseRepository
{
    ...
    {
        $treeTable = 'shop_category_tree';
        $itemTable = 'shop_category_items';
    ...
    public function updateChildrenPathName(int $domainId, string $oldPathName, string $newPathName): void
    {
        ...
        $this->getDb()->execute($sql, [
            $newPathName,
            // SUBSTRING 은 문자 단위로 센다. strlen() 은 바이트라 한글 카테고리명에서
            // 시작 위치가 밀려 하위 경로가 통째로 잘린다.
            mb_strlen($oldPathName) + 1,
            $domainId,
            $oldPathName . '>%',
        ]);
    }
