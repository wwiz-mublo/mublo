...
class ShopSitemapProvider implements SitemapUrlProviderInterface
{
     ...
     * 만들므로 그 규칙을 그대로 따른다.
     *
     * 접근 제한(allow_member_level>0)·성인 인증(is_adult=1) 카테고리에 속한 상품은
     * 공개 선언 대상이 아니라고 보고 뺀다(계약 1번). 주/보조 카테고리 양쪽을 본다.
     *
    ...
    private function productUrls(int $domainId): iterable
    {
        ...
        // 카테고리 테이블이 없으면 제한 판정 자체가 불가능 → 필터 없이 진행
        $restricted = '';
            ...
            $restricted = "
                AND NOT EXISTS (
                    SELECT 1
                     ...
                     WHERE ci.domain_id = g.domain_id
                       AND ci.category_code IN (g.category_code, g.category_code_extra)
