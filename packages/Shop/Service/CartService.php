...
class CartService
{
     ...
     *
     * 판매가·적립 계산에 넘겨야 한다 — 빠뜨리면 "쇼핑몰 기본설정 적용" 상품의
     * 할인이 상품 화면에만 걸리고 장바구니·주문에는 빠진다.
     */
    ...
    public function getCartList(string $sessionId, int $memberId, int $domainId): Result
    {
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if ($cartItem->hasOption() && $cartItem->getOptionId() > 0) {
                // getOptionMode() 는 backed enum 을 반드시 반환한다. ?? 뒤의 문자열
                // 캐스팅은 닿지 않는 길이고, 닿으면 enum 캐스팅이라 죽는다.
    ...
    public function getGroupedCartList(string $sessionId, int $memberId, int $domainId): Result
    {
        ...
        foreach (array_keys($productDataIds) as $gid) {
            ...
            // 프론트 전달 형태는 상품 상세와 동일하게 ProductOptionPresenter가 정한다.
            ...
            $productData[$gid] = [
                ...
                'options' => ProductOptionPresenter::toOptionList($flatOptions),
                'combos' => $optionMode === 'COMBINATION'
                    ? ProductOptionPresenter::toComboList($this->productOptionRepository->getCombos($gid))
                    : [],
            ];
        }
     ...
     */
    private function isCartItemOwner(CartItem $cartItem, string $cartSessionId, int $memberId): bool
    {
        // 회원: member_id 일치
