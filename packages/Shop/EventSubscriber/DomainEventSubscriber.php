...
class DomainEventSubscriber implements EventSubscriberInterface
{
    ...
    private SiteProvisioningInterface $menuService;
    private MenuManagementInterface $menuItemRepo;
    private ShippingService $shippingService;
    private ShopConfigService $shopConfigService;

    public function __construct(
        SiteProvisioningInterface $menuService,
        MenuManagementInterface $menuItemRepo,
    ...
    public function onDomainCreated(DomainCreatedEvent $event): void
    {
        ...
        $existing = $this->menuItemRepo->findProviderMenus($domainId, 'package', 'Shop');
        if (!empty($existing)) {
            return;
        }
     ...
     */
    public static function seedMenus(SiteProvisioningInterface $menuService, int $domainId): void
    {
        // 프론트 메뉴
        $menuService->createMenuItem($domainId, [
            'label' => '쇼핑',
            'url' => '/shop',
