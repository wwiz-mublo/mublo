...
class BoardGroupRenderer implements RendererInterface
{
    ...
    public function render(BlockColumnView $column): string
    {
        ...
        if (!empty($groupIds)) {
            ...
            // 선택된 그룹(들)에 속한 활성 게시판 수집
            // findByGroup()에서 가져온 엔티티를 재사용해 게시판별 재조회를 피한다.
            $boards = [];
            foreach ($groupIds as $gid) {
                $candidate = $gid === $groupIds[0]
                    ? $group
                    : $this->groupRepository->findByIdForDomain($column->getDomainId(), $gid);

                if (!$candidate) {
                    continue;
                }
                foreach ($this->getBoardsByGroup($gid, $column->getDomainId()) as $board) {
            ...
            if (!empty($boards)) {
                // 게시판별 최신글 조회
                $boardsData = $this->getBoardsWithArticles(array_values($boards), $column->getDomainId(), $count);
            }
        }
    ...
    /**
     * 그룹 내 현재 도메인의 활성 게시판 목록 조회
     *
     * @return BoardConfig[]
     */
    private function getBoardsByGroup(int $groupId, int $domainId): array
    {
        $boards = $this->boardRepository->findByGroup($groupId);

        return array_values(array_filter(
            $boards,
            fn(BoardConfig $board): bool => $board->getDomainId() === $domainId && $board->isActive()
        ));
    }
     ...
     * 게시판별 최신글 데이터 조회
     *
     * @param BoardConfig[] $boards
     */
    private function getBoardsWithArticles(array $boards, int $domainId, int $count): array
    {
        $result = [];

        foreach ($boards as $board) {
            $articlesResult = $this->articleRepository->getRecentList(
                $domainId,
                $board->getBoardId(),
                $count,
                ['status' => 'published'],
