...
class BoardAttachmentRepository extends BaseRepository
{
    ...
    {
        return $this->findWithArticleBy('f.attachment_id', $attachmentId);
    }

    /**
     * 공개 식별자로 조회. 다운로드 경로가 쓰는 진입점이다.
     */
    public function findWithArticleByPublicId(string $publicId): ?array
    {
        return $this->findWithArticleBy('f.public_id', $publicId);
    }

    private function findWithArticleBy(string $column, int|string $value): ?array
    {
