...
class RecentNoticesWidget extends AbstractDashboardWidget
{
    ...
    {
        return [
            ['type' => 'style', 'src' => '/serve/package/Board/assets/css/admin.css'],
        ];
    }
    ...
    public function render(): string
    {
        ...
        foreach ($notices as $notice) {
            ...
            $html .= '<a href="/admin/board/article/view?id=' . $articleId . '&amp;activeCode=K_Board_005" class="board-recent-notice list-group-item list-group-item-action px-3 py-2">';
            $html .= '<div class="text-truncate small fw-medium">' . $title . '</div>';
            $html .= '<div class="d-flex flex-wrap align-items-center gap-2 text-muted mt-1" style="font-size:0.7rem">';
            $html .= '<span><i class="bi bi-person me-1"></i>' . $author . '</span>';
            $html .= '<span><i class="bi bi-clock me-1"></i>' . $date . '</span>';
            $html .= '<span><i class="bi bi-eye me-1"></i>' . $views . '</span>';
            $html .= '</div>';
            $html .= '</a>';
        }
        $html .= '</div>';
