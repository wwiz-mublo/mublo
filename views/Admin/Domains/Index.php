 ...
 * @var int $currentDomainId 지금 접속 중인 도메인 ID (0이면 없음)
 * @var bool $isSuper 최고관리자 여부 (호스트명 변경 가능)
 */
$settingsLinks = $settingsLinks ?? [];
$domainPackagesMap = $domainPackagesMap ?? [];
$domainOwnerMap = $domainOwnerMap ?? [];
$currentDomainId = (int) ($currentDomainId ?? 0);
...
foreach ($domains as $domain) {
    ...
    // 현재 접속 도메인 행은 일괄 수정·삭제 대상이 아니다 (서버도 self를 거부한다).
    // checkbox 컬럼의 skip_key로 체크박스 자체를 렌더하지 않는다.
    $row['is_current'] = $domain->getDomainId() === $currentDomainId;
    $domainsData[] = $row;
}
...
$columns = $this->columns()
    ...
    ->callback('domain', '도메인', function ($row) {
        ...
        if (!empty($row['is_current'])) {
            $badge .= '<span class="badge bg-primary ms-1">현재 접속</span>';
            // 아래 스크립트가 이 마커로 행을 찾아 상태 셀렉트를 잠근다.
            $badge .= '<span class="js-current-row d-none"></span>';
        }
    ...
    ->actions('actions', '관리', function ($row) use ($settingsLinks, $domainPackagesMap, $editReturn, $isSuper) {
        $id = $row['domain_id'];
        $domain = htmlspecialchars($row['domain']);
        $status = $row['status'] ?? '';
        $isCurrent = !empty($row['is_current']);

        // 현재 접속 도메인: 이미 그 사이트에 있으므로 접속 버튼이 없고, 자기 자신은 삭제 대상도 아니다.
        // 호스트명 변경 권한이 없으면 바꿀 게 없으므로 조회('정보')로 표기한다.
        if ($isCurrent) {
            return '<a href="/admin/domains/edit/' . $id . '?return=' . $editReturn . '" class="btn btn-sm btn-default">'
                . ($isSuper ? '수정' : '정보') . '</a>';
        }
...
<div class="page-container">
    <div class="page-title">
        <div class="page-title-text">
            ...
            <p>도메인을 관리합니다. 현재 사이트 설정은 <a href="/admin/settings" data-no-active-code>기본 설정</a>에서 변경하세요.</p>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 현재 접속 도메인 행: 상태 인라인 변경을 잠근다.
    // (체크박스가 없어 일괄 수정에 담기지도 않으므로, 바꿀 수 있어 보이면 오해만 준다)
    document.querySelectorAll('.js-current-row').forEach(function(marker) {
        var row = marker.closest('tr');
        if (!row) return;
        row.querySelectorAll('select').forEach(function(sel) {
            sel.disabled = true;
            sel.title = '현재 접속 중인 사이트의 상태는 이 목록에서 변경할 수 없습니다.';
        });
    });
