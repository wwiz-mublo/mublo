...
<div class="card">
    <div class="card-hero">
        <i class="bi bi-person-lines-fill text-pastel-purple"></i>
        <span>마이페이지 사이드바 메뉴</span>
        <small class="text-muted fw-normal">
            <i class="bi bi-lock-fill"></i> 잠금 표시된 시스템 메뉴는 제거할 수 없습니다.
        </small>
        <button type="button" class="btn btn-primary btn-sm ms-auto" id="btn-save-mypage">
    ...
    <div class="card-body p-0">
        <div class="row g-0">
            ...
            <div class="col-md-7 p-3">
                ...
                <div id="mypage-list" style="min-height:48px; background:var(--bs-secondary-bg); border-radius:6px; padding:6px;">
                    <p class="menu-empty-msg text-muted text-center py-4 small mb-0<?= empty($mypageMenus) ? '' : ' d-none' ?>">여기에 메뉴를 드래그하세요</p>
                    <?php foreach ($mypageMenus as $menu): ?>
                    <?php $pt = $menu['provider_type'] ?? 'core'; $pn = $menu['provider_name'] ?? ''; ?>
                    <?php $isSys = (bool)($menu['is_system'] ?? 0); ?>
                    <div class="menu-active-item d-flex align-items-center gap-2 p-2 mb-1 border rounded<?= $isSys ? ' is-system' : '' ?>"
                        ...
                        <?php if ($isSys): ?>
                        <i class="bi bi-lock-fill text-muted" style="font-size:1.1rem; min-width:1.1rem;" title="시스템 메뉴"></i>
                        <?php else: ?>
                        ...
                        <span class="flex-grow-1 d-flex align-items-center gap-2" style="font-size:0.9rem">
                            ...
                            <?php if ($isSys): ?>
                            <span class="badge bg-secondary-subtle text-secondary-emphasis border border-secondary-subtle small">시스템</span>
                            <?php endif; ?>
