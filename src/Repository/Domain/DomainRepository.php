...
class DomainRepository extends BaseRepository
{
    ...
    private function applyFilterConditions(object $query, array $filters): void
    {
        ...
        // 자신 + 하위 도메인 필터 (자기 도메인도 목록에 포함한다)
        //
        // LIKE 'X%' 한 방으로 합치지 않는 이유: 그룹은 루트 도메인 ID로 시작하므로
        // '1'로 LIKE 하면 별개 루트인 '10', '15'까지 걸린다. 등가·접두 두 조건을 명시한다.
        if (!empty($filters['self_or_child_of_domain_group'])) {
            $group = (string) $filters['self_or_child_of_domain_group'];
            $query->whereRaw(
        ...
        // 단일 도메인 한정 (그룹 정보가 없어 계층 판정이 불가능할 때의 fail-closed 경로)
