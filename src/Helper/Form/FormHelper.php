...
class FormHelper
{
     ...
     * 업로드된 파일의 유효성 검증
     *
     * 판정은 코어 allowlist(UploadPolicy)가 한다. 예전에는 이 메서드가 자체 검사를
     * 했는데, options 로 받은 MIME 목록만 보고 확장자와 실제 내용이 일치하는지는
     * 확인하지 않았다. 코어 관문보다 헐거운 검사가 "업로드 검증 헬퍼"라는 이름으로
     * 놓여 있으면, 이걸 쓰는 쪽이 더 안전하다고 믿으면서 실제로는 덜 안전해진다.
     *
     * options 는 코어 허용 범위 안에서 좁히는 용도로만 쓴다. 넓힐 수는 없다.
     *
     * @param array $file 단일 파일 정보 배열
     * @param array $options 검증 옵션
     *   - maxSize: 최대 파일 크기 (bytes)
     *   - allowedTypes: 허용 MIME 타입 배열 (코어 허용 범위 안에서 좁히기)
     *   - allowedExtensions: 허용 확장자 배열 (코어 허용 범위 안에서 좁히기)
    ...
    public static function validateUploadedFile(array $file, array $options = []): array
    {
        ...
        if (isset($options['maxSize']) && ($file['size'] ?? 0) > $options['maxSize']) {
        ...
        // 이름만 보고 거를 수 있는 것을 먼저 — 파일을 열지 않아도 되고 사유가 분명하다
        $extension = strtolower(pathinfo((string) ($file['name'] ?? ''), PATHINFO_EXTENSION));

        // 기능단 좁히기 — 코어 목록 밖으로 넓히지는 못한다
        if (!empty($options['allowedExtensions'])
            && !in_array($extension, array_map('strtolower', $options['allowedExtensions']), true)) {
            return ['valid' => false, 'error' => '허용되지 않는 파일 확장자입니다.'];
        }

        $tmpName = (string) ($file['tmp_name'] ?? '');
        if ($tmpName === '' || !is_readable($tmpName)) {
            return ['valid' => false, 'error' => '업로드 파일을 찾을 수 없습니다.'];
        }

        $mimeType = null;
        if (class_exists('finfo')) {
            $detected = (new \finfo(FILEINFO_MIME_TYPE))->file($tmpName);
            $mimeType = $detected === false ? null : $detected;
        }

        if (!empty($options['allowedTypes'])
            && ($mimeType === null || !in_array($mimeType, $options['allowedTypes'], true))) {
            return ['valid' => false, 'error' => '허용되지 않는 파일 형식입니다.'];
        }

        // 코어 판정: 확장자가 allowlist 에 있고 실제 내용과 일치해야 한다.
