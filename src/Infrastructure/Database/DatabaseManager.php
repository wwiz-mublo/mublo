...
class DatabaseManager
{
    ...
    {
        // DB 설정은 없으면 아무것도 못 한다 — 부재를 기본값으로 덮지 않는다.
        if (!ConfigFile::exists('database')) {
            throw DatabaseException::configError('Database config file not found: config/database.php');
        }

        $rawConfig = ConfigFile::load('database');
