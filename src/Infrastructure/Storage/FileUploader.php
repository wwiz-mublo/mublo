...
class FileUploader
{
    ...
    public function adoptFile(string $sourceFullPath, int $domainId, array $options = []): UploadResult
    {
        ...
        // 이미지 메타 추출 + 위조 차단. 한 번 파싱해 둘 다에 쓴다(upload()와 동일).
        // 결과를 버리면 치수가 필요한 호출자가 같은 파일을 다시 읽어야 한다.
        $imageInfo = @getimagesize($sourceFullPath);
        $imageWidth = is_array($imageInfo) ? ($imageInfo[0] ?? null) : null;
        $imageHeight = is_array($imageInfo) ? ($imageInfo[1] ?? null) : null;

        // 래스터 이미지 확장자는 실제 이미지로 파싱되는지까지 확인(스크립트가 든 폴리글롯 차단)
        if ($imageWidth === null && in_array($extension, $this->rasterImageExtensions, true)) {
            return UploadResult::failure("이미지 파일 형식이 올바르지 않습니다: {$extension}");
        }
