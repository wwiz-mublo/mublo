...
class SecureFileService
{
     ...
     * 다운로드 토큰 서명 키 로드
     *
     * 이 키는 CSRF 와 무관하다. CSRF 토큰은 세션에 담은 난수를 대조하는 방식이라
     * 키를 쓰지 않는다(CsrfManager). 그런데 이 서명 키가 오랫동안 security.csrf.token_key
     * 자리에 있었고 설치 화면도 'CSRF 토큰 키'로 표시했다. "CSRF 키니까 바꿔도
     * 되겠지" 하고 재생성하면 발급된 다운로드 링크가 전부 죽는 자리였다.
     *
     * 이름을 security.file.download_signing_key 로 옮기고, 기존 설치가 깨지지 않게
     * 옛 위치를 폴백으로 읽는다.
     */
    private function loadSecretKey(): string
    {
        // 설치 전에는 config 가 없을 수 있어 부재는 허용한다. 다만 읽기 실패까지
        // 삼키면 아래 랜덤 폴백으로 조용히 넘어가, 이미 발급한 다운로드 토큰이
        // 전부 무효가 된 이유를 아무 데서도 찾을 수 없게 된다. ConfigFile 이
        // 그 계약을 보장한다.
        $security = ConfigFile::load('security');

        $signingKey = $security['file']['download_signing_key'] ?? '';
        if (is_string($signingKey) && $signingKey !== '') {
            return $signingKey;
        }

        // 폴백: 이름을 바꾸기 전에 설치된 사이트. 값을 옮기지 않아도 계속 동작해야
        // 기존 다운로드 링크가 살아 있다.
        $legacyKey = $security['csrf']['token_key'] ?? '';
