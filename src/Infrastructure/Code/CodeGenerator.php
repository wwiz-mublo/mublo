...
class CodeGenerator
{
    ...
    public function sequential(string $codeType, string $prefix, int $padding = 6): string
    {
        ...
        // MAX(...) 는 드라이버에 따라 string/float 로 온다. strict_types 아래서
        // str_pad() 에 그대로 넘기면 TypeError 이므로 int 로 좁힌 뒤 문자열화한다.
        $nextSeq = (int) ($result['max_seq'] ?? 0) + 1;
        $code = $prefix . str_pad((string) $nextSeq, $padding, '0', STR_PAD_LEFT);
