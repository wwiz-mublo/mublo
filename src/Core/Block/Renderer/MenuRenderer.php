 ...
 *
 * 행 HTML 캐시는 row_id 단위다. 블록 페이지의 행은 페이지 URL 이 고정이라
 * 캐시돼도 정확하지만, **공용 위치(position)의 행에 scope=current 를 두면
 * 최초 렌더 시점의 URL 기준으로 캐시**되어 다른 페이지에서 부정확할 수 있다.
 * scope=current 는 블록 페이지 안에서 쓰는 것을 전제로 하며, 스킨의 보조
 * 스크립트가 active 표시를 클라이언트에서 재동기화한다.
 */
