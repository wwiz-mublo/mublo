...
class ExtensionManager
{
    ...
    private function bootProviders(Context $context): void
    {
        ...
        $this->drainBlockTypeConflicts();
    }

    /**
     * 블록 타입 코드 선점으로 생략된 등록을 확장 진단에 합류시킨다.
     *
     * BlockRegistry는 부팅을 멈추지 않으려고 충돌을 조용히 넘긴다. 그 조용함이
     * 곧 "확장이 이유 없이 반쪽만 동작한다"로 보이므로, 진단 경로에는 남긴다.
     */
    private function drainBlockTypeConflicts(): void
    {
        foreach (BlockRegistry::getTypeConflicts() as $conflict) {
            [$type, $name] = $this->splitOwner($conflict['claimant']);

            $message = sprintf(
                '블록 콘텐츠 타입 "%s"(%s) 등록이 생략됐습니다. 이미 %s이(가) 사용 중입니다.',
                $conflict['type'],
                $conflict['claimantTitle'],
                $conflict['holder'] ?? $conflict['holderKind']
            );

            $this->loadErrors[] = [
                'type' => $type,
                'name' => $name,
                'stage' => 'block-type-conflict',
                'message' => $message,
            ];
    ...
    /**
     * 'package:Shop' 형태의 소유자 키를 종류와 이름으로 되돌린다.
     *
     * @return array{0: string, 1: string}
     */
    private function splitOwner(?string $owner): array
    {
        if ($owner === null || !str_contains($owner, ':')) {
            return ['extension', $owner ?? 'unknown'];
        }
