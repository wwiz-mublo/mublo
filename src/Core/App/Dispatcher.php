...
class Dispatcher
{
    ...
    protected function createController(string $controllerClass): object
    {
        ...
        foreach ($parameters as $param) {
            // isBuiltin()/getName() 은 ReflectionNamedType 의 것이다. union·intersection
            // 타입이면 ReflectionType 만 오므로 좁히지 않고 부르면 그 자리에서 죽는다.
            // 주입 대상이 하나로 정해지지 않는 타입은 해결하지 않고 넘긴다.
            $type = $param->getType();
    ...
    protected function invokeAction(
        ...
        foreach ($methodParams as $param) {
            // union·intersection 타입에는 isBuiltin()/getName() 이 없다. 좁히지 않고
            // 부르면 그 액션을 가진 컨트롤러가 통째로 500 이 된다. 하나로 정해지지
            // 않는 타입은 Request/Context 주입 대상이 아니므로 아래 이름 기반 분기로
            // 흘려보낸다.
            $type = $param->getType();
            $type = $type instanceof \ReflectionNamedType ? $type : null;
            $name = $param->getName();
