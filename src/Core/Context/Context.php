...
class Context
{
    ...
    /**
     * 이 요청의 도메인이 SUPER 도메인인가
     *
     * 도메인 그룹은 `{superId}/{...}` 계층이고, SUPER 는 그룹의 첫 ID 가
     * 자기 자신인 도메인이다. Router 의 super_only 판정과 같은 규칙을
     * 한 곳으로 모은다 — 흩어져 재구현되면 판정이 갈라진다.
     */
    public function isSuperDomain(): bool
    {
        $domainId = $this->getDomainId();
        $group = (string) ($this->getDomainGroup() ?? '');

        if ($domainId === null || $group === '') {
            return false;
        }

        return (int) explode('/', $group)[0] === $domainId;
    }
    ...
    /** CLI 등 안정 Domain 조회 계약을 사용하는 진입점에서 요청 문맥을 구성한다. */
    public function setDomainDescriptor(DomainDescriptor $domain): void
    {
        $this->domainInfo = Domain::fromArray([
            'domain_id' => $domain->domainId,
            'domain' => $domain->hostname,
            'domain_group' => $domain->domainGroup,
            'member_id' => $domain->ownerMemberId,
            'status' => $domain->status,
            'site_config' => ['site_title' => $domain->siteTitle],
            'company_config' => $domain->companyConfig,
            'seo_config' => $domain->seoConfig,
        ]);
