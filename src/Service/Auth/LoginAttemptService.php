...
{
    /**
     * 판정 시점의 집계에는 방금 기록한 이번 시도가 이미 들어 있다. 그래서 한도에
     * 한 칸 여유를 둔다 — 한도 5 라면 5회째 시도는 허용하고 6회째부터 거부해야
     * 하는데, 5회째 시점의 집계값이 이미 5 이기 때문이다.
     */
    private const OWN_ATTEMPT = 1;
    ...
    /**
     * 시도를 먼저 기록하고 나서 한도를 판정한다.
     *
     * check() 로 세고 나중에 record() 하면 그 사이가 비어 있다. 동시에 들어온 요청이
     * 전부 집계를 끝낸 뒤에야 첫 기록이 들어가므로, 병렬로 던지면 한 번에 한도보다
     * 훨씬 많은 시도가 통과한다. 카운트가 유실되지는 않아 한 차례로 끝나지만,
     * 그 한 차례가 5회를 수십 회로 만든다.
     *
     * 기록을 먼저 하면 뒤이은 집계가 서로를 본다. 잠금 없이 순서만 바꿔 닫는다.
     *
     * 실패로 기록해 두고 판정하므로, 로그인이 성공하면 호출자가 record(success: true)
     * 로 마무리한다 — 그 안의 clearFailedAttempts() 가 여기서 넣은 행까지 지운다.
     *
     * 요청이 도중에 죽어도 기록은 남는다. 집계가 사라지던 이전보다 안전한 쪽이다.
     *
     * @return array{allowed: bool, message: string, remaining: int, retry_after: int}
     */
    public function registerAttempt(int $domainId, string $userId, string $ipAddress): array
    {
        ...
        try {
            $this->doRecord($domainId, $userId, $ipAddress, false);

            return $this->doCheck($domainId, $userId, $ipAddress);
        } catch (\Throwable $e) {
            // DB 장애로 판정할 수 없으면 로그인을 막지 않는다(가용성). 다만 브루트포스
            // 보호가 조용히 꺼지지 않게 반드시 남긴다.
            error_log('[SECURITY] LoginAttemptService::registerAttempt 실패 — 브루트포스 보호 일시 비활성: ' . $e->getMessage());
            return ['allowed' => true, 'message' => '', 'remaining' => 999, 'retry_after' => 0];
        }
    }
    ...
    private function doCheck(int $domainId, string $userId, string $ipAddress): array
    {
        ...
        if ($userAttempts >= $maxUser + self::OWN_ATTEMPT) {
        ...
        if ($ipAttempts >= $maxIp + self::OWN_ATTEMPT) {
        ...
        return [
            'allowed' => true,
            'message' => '',
            // 방금 쓴 이번 시도를 뺀 "앞으로 남은" 횟수. 집계에 자기 행이 들어 있으므로
            // 여기에는 여유분을 더하지 않는다 — 5회 중 4회째면 남은 것은 1회다.
            'remaining' => max(0, $maxUser - $userAttempts),
            'retry_after' => 0,
        ];
    }
