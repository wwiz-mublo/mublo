...
class BlockKitLibrary
{
    ...
    public function apply(
        ...
        $result = $this->applier->apply($domainId, $kit, $mode, $applySiteSettings, $allowScripts);
        if (!$result['ok']) {
            return $result;
        }

        $result['application_id'] = $this->applicationRepository->create(
            $this->toApplicationRow($domainId, $kitId, $mode, $result['summary'] ?? [], $appliedBy)
        );
    ...
    public function rollback(int $domainId, int $applicationId): array
    {
        ...
        if ($result->isFailure()) {
            return ['ok' => false, 'errors' => ['사이트 설정을 되돌리지 못했습니다: ' . $result->getMessage()], 'restored' => []];
        }

        $this->applicationRepository->clearSnapshot($applicationId);
