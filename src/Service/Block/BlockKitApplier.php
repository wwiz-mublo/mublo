...
class BlockKitApplier
{
    ...
    public function dryRun(array $kit, array $context = []): array
    {
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            foreach ($columns as $columnIndex => $column) {
                ...
                if (is_string($contentType) && $contentType !== '' && !BlockRegistry::hasContentType($contentType)) {
                ...
                // "설정이 필요한 칸" 체크리스트(설계 8.3). kit_hint 는 DB 에 저장하지 않고
                // 적용 결과 화면에서만 쓴다.
                $setupReason = $this->resolveSetupReason($column);
                if ($setupReason !== null) {
                    // 인덱스는 블록 킷이 통제하는 값이다(rows 를 객체로 주면 키가 문자열이 된다).
                    // 관리자 화면이 그대로 innerHTML 로 찍으므로 반드시 정수로 강제한다.
                    $needsSetup[] = [
                        ...
                        'reason' => $setupReason,
                        // 확장이 없는 칸에만 "무엇을 설치하라"고 말할 수 있다. 이미 형식을 검증했다.
                        'provider' => $setupReason === 'extension_missing' && is_string($contentType)
                            ? ($providers[$contentType]['label'] ?? null)
                            : null,
                        'kit_hint' => isset($column['kit_hint']) && is_string($column['kit_hint'])
                            ? $column['kit_hint']
                            : null,
                    ];
                }
    ...
    {
        $config = $column['content_config'] ?? null;
        if (!is_array($config)) {
            return false;
        }
        ...
        foreach ($rows as $row) {
            ...
            if ($this->revisionRepository !== null) {
                $this->revisionRepository->create(
                    ...
                    [
                        'row' => $row->toArray(),
                        'columns' => array_map(static fn ($column) => $column->toArray(), $columns),
                    ],
                    'kit_replace'
                );
