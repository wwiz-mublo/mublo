...
class BlockContentSanitizer
{
    ...
    {
        $config = $this->normalizeContentConfig($column['content_config'] ?? null);
    ...
    /**
     * column create/update payload의 HTML content_config를 정화한다.
     */
    public function sanitizeColumnData(array $data): array
    {
        if (($data['content_type'] ?? '') !== BlockContentType::HTML->value) {
            return $data;
        }

        $wasString = is_string($data['content_config'] ?? null);
        $contentConfig = $this->normalizeContentConfig($data['content_config'] ?? null);

        if (!is_array($contentConfig)) {
            return $data;
        }

        $contentConfig = $this->sanitizeHtmlConfig($contentConfig);

        $data['content_config'] = $wasString
