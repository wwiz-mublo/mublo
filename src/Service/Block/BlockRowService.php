...
class BlockRowService
{
    ...
    {
        if ($this->columnWriter !== null) {
            return $this->columnWriter->columnsToPortableArrays($columns);
        }

        // 레거시 수동 생성 환경의 폴백. 운영 DI에서는 columnWriter가 공통 배치
        // 직렬화를 담당해 일반 revision과 Kit 교체 revision이 같은 형식을 쓴다.
        $result = [];
    ...
    ): Result
    {
        ...
        if (!$normalizedColumns->isOk()) {
            return Result::failure(
                ...
                [
                    ...
                    'warnings' => $normalizedColumns->getWarnings(),
                ]
            );
        }
        $columnsData = $normalizedColumns->getNormalizedColumns();
        $normalizationWarnings = $normalizedColumns->getWarnings();
        ...
        return Result::success('행이 생성되었습니다.', [
            'row_id' => $rowId,
    ...
    ): Result
    {
        ...
        $effectivePageId = array_key_exists('page_id', $data)
            ? (int) ($data['page_id'] ?? 0)
            : (int) ($row->getPageId() ?? 0);
        $effectivePosition = array_key_exists('position', $data)
            ? trim((string) ($data['position'] ?? ''))
            : ($row->getPosition()?->value ?? '');
        if (($effectivePageId > 0) === ($effectivePosition !== '')) {
            return Result::failure('출력 위치와 페이지 중 하나만 지정해야 합니다.');
        }
        ...
        return Result::success('행이 수정되었습니다.', ['warnings' => $normalizationWarnings]);
    }
    ...
    public function updateOrder(array $rowIds, ?int $domainId = null): Result
    {
        ...
        try {
            $this->db->transaction(function () use ($rowIds): void {
                if (!$this->repository->updateOrder($rowIds)) {
                    throw new \RuntimeException('정렬 대상 행이 변경되었거나 존재하지 않습니다.');
                }
            });
            ...
            return Result::failure('정렬 순서 변경에 실패했습니다. 목록을 새로고침한 뒤 다시 시도해 주세요.');
        }
    }
    ...
    public function setOrder(array $orders, ?int $domainId = null): Result
    {
        ...
        $rowsToInvalidate = [];
        try {
            $this->db->transaction(function () use ($orders, &$rowsToInvalidate): void {
                foreach ($orders as $rowId => $sortOrder) {
                    $rowId = (int) $rowId;
                    $row = $this->repository->find($rowId);
                    if (!$row) {
                        throw new \RuntimeException('정렬 대상 행을 찾을 수 없습니다.');
                    }
                    ...
                    if ($this->repository->updateIfRevision(
                        ...
                        throw new \RuntimeException('정렬 대상 행이 다른 요청에서 변경되었습니다.');
                    }
        ...
        $updated = count($rowsToInvalidate);
    ...
    public function restoreRevision(
        ...
        if (!$normalized->isOk()) {
            return Result::failure($normalized->getFirstErrorMessage(), [
                'errors' => $normalized->getErrors(),
                'warnings' => $normalized->getWarnings(),
            ]);
        }
        $normalizationWarnings = $normalized->getWarnings();
        ...
        return Result::success('선택한 이력으로 복구했습니다.', [
            'row_id' => $rowId,
            'warnings' => $normalizationWarnings,
        ]);
    }
    ...
    public function copyRow(
        ...
        return Result::success('행이 복사되었습니다.', [
            'row_id' => $newRowId,
            'warnings' => $normalizationWarnings,
        ]);
    }
