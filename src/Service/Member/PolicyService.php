...
class PolicyService implements PolicyQueryInterface
{
    ...
    public function findDocument(int $domainId, int $policyId): ?PolicyDocument
    {
        ...
        if ($policy === null || $policy->getDomainId() !== $domainId) {
            return null;
        }

        return $this->documentsForPolicies([$policy])[0] ?? null;
    }
    ...
    public function create(int $domainId, array $data): Result
    {
        ...
        try {
            ...
            // 사용자에게는 고정 문구를 주되 원인은 남긴다. 예외를 통째로 버리면
            // 실패가 조용해져서 도메인 생성이 막혔을 때 원인을 추적할 수 없다.
            error_log(sprintf(
                '[PolicyService] policy_create_failed domain_id=%d slug=%s error=%s',
                $domainId,
                $slug,
                $e->getMessage()
            ));
    ...
    public function update(int $policyId, array $data): Result
    {
        ...
        try {
            ...
            error_log(sprintf(
                '[PolicyService] policy_update_failed policy_id=%d domain_id=%d version=%s error=%s',
                $policyId,
                $domainId,
                $newVersion,
                $e->getMessage()
    ...
    public function updateSortOrders(array $orderData): Result
    {
        ...
        try {
            ...
            error_log(sprintf(
                '[PolicyService] policy_sort_failed policy_ids=%s error=%s',
    ...
    public function seedDefaultPolicies(int $domainId): Result
    {
        ...
        foreach ($defaults as $def) {
            ...
            if ($result->isFailure()) {
                // 실패 사유를 버리면 도메인 생성 실패 로그에 약관 제목만 남아
                // 무엇이 잘못됐는지 알 수 없다. create() 의 판단을 그대로 전달한다.
                return Result::failure(
                    "기본 약관 생성 실패: {$def['title']} ({$result->getMessage()})"
                );
            }
            $created++;
        }
    ...
    private function documentsForPolicies(array $policies): array
    {
        ...
        foreach ($policies as $policy) {
            ...
            $documents[] = new PolicyDocument(
                ...
                type: $policy->getPolicyType()->value,
                typeLabel: $policy->getPolicyTypeLabel(),
            );
        }
