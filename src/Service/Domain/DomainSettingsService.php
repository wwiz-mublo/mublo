...
class DomainSettingsService implements CompanyInfoInterface
{
    ...
    public function updateThemeConfig(int $domainId, array $config): Result
    {
        ...
        // super_ 스킨은 SUPER 전용이다 (ThemeSkinPolicy 관례). 목록에서도
        // 걸러지지만, 저장의 문에서 막아야 조작이 통하지 않는다.
        // 실존 검사보다 먼저 본다 — 전용 스킨은 존재 여부와 무관하게 거부다.
        if (!$this->isSuperDomain($domain)) {
            foreach ($themeSelection as $key => $skin) {
                if (ThemeSkinPolicy::isSuperOnly($skin)) {
                    return Result::failure('SUPER 도메인 전용 스킨입니다.', [
                        'errors' => ["theme.{$key}" => 'SUPER 도메인 전용 스킨입니다.'],
                    ]);
                }
            }
        }
    ...
    /**
     * SUPER 도메인 판정 — Context::isSuperDomain() 과 같은 규칙
     *
     * 여기는 엔티티만 있고 요청 문맥이 없어 같은 판정을 엔티티로 한다.
     */
    private function isSuperDomain(\Mublo\Entity\Domain\Domain $domain): bool
    {
        $group = (string) $domain->getDomainGroup();
        if ($group === '') {
            return false;
        }

        return (int) explode('/', $group)[0] === $domain->getDomainId();
    }
}
