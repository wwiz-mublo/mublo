...
class DomainService
{
    ...
    /**
     * 회원 ID 목록 → 회원아이디(user_id) 맵
     *
     * 감사 표시용(도메인 변경 이력의 실행자 아이디). 삭제된 회원은 맵에서 빠지므로
     * 호출부는 ID를 그대로 노출하는 등의 폴백을 가져야 한다.
     *
     * @param int[] $memberIds
     * @return array<int,string> [member_id => user_id]
     */
    public function getUserIdMapByMemberIds(array $memberIds): array
    {
        if (!$this->memberRepository) {
            return [];
        }
        ...
        $map = [];
        foreach ($this->memberRepository->findByIds($ids) as $member) {
            $map[$member->getMemberId()] = $member->getUserId();
        }
    ...
    ): Result
    {
        ...
        // 중복 체크 (입력값·포트 제거형 모두 대조 — isDomainTaken 단일 경로)
    ...
    public function update(int $domainId, array $data): Result
    {
        ...
        if (isset($data['domain']) && $data['domain'] !== $domain->getDomain()) {
            ...
            // 중복 체크 (자기 자신 제외 — isDomainTaken 단일 경로)
            if ($this->isDomainTaken($data['domain'], $domainId)) {
                return $this->fail('이미 등록된 도메인입니다.');
            }
        }
    ...
    /**
     * 도메인 호스트명 변경 (검증 통과 필수)
     *
     * update()가 아니라 별도 경로를 두는 이유:
     * 호스트명은 사이트의 주소 그 자체라, 잘못 바꾸면 그 사이트가 즉시 접속 불가가 된다.
     * 그래서 일반 수정과 달리 "그 호스트가 실제로 이 서버에 도달한다"는 검증
     * (DomainVerificationService)을 통과한 기록이 있어야만 저장한다. 검증 기록은
     * 1회만 소진되므로 확인 없이/재사용해서 바꿀 수 없다.
     *
     * 부수 정리:
     * - 도메인 해석 캐시: update()가 구·신 도메인 양쪽을 무효화한다.
     * - 라우트 캐시: 파일명이 도메인 문자열 기반이라 옛 도메인 캐시를 직접 지운다.
     * - 스토리지(D{id})·데이터 캐시(d{id})는 도메인 ID 기준이라 영향 없다.
     *
     * 호출자(Controller)가 권한(슈퍼관리자)과 계층을 먼저 검증해야 한다.
     *
     * @param int $domainId 대상 도메인 ID
     * @param string $newDomain 새 호스트명
     * @param int|null $actorMemberId 변경을 실행한 관리자 회원 ID (감사용)
     */
    public function changeDomainName(int $domainId, string $newDomain, ?int $actorMemberId = null): Result
    {
        $domain = $this->domainRepository->find($domainId);
        if (!$domain) {
            return $this->fail('도메인을 찾을 수 없습니다.');
        ...
        $oldDomain = $domain->getDomain();
        $newHost = DomainVerificationService::normalizeHost($newDomain);

        if ($newHost === '') {
            return $this->fail('변경할 도메인명을 입력해주세요.');
        }

        if ($newHost === strtolower($oldDomain)) {
            return $this->fail('현재 도메인과 동일합니다.');
        }

        // 형식 검증
        $validation = $this->validateDomainName($newHost);
        if ($validation->isFailure()) {
            return $validation;
        }

        // 중복 검증 (자기 자신 제외 — isDomainTaken 단일 경로).
        // 검증 게이트보다 앞에 둬서, 이미 쓰이는 호스트명으로 통과 기록을 소진하지 않게 한다.
        if ($this->isDomainTaken($newHost, $domainId)) {
            return $this->fail('이미 등록된 도메인입니다.');
        }

        // 검증 게이트 — 통과 기록이 없으면 여기서 반려된다.
        if (!$this->verificationService) {
            return $this->fail('도메인 검증 서비스를 사용할 수 없어 변경할 수 없습니다.');
        }

        // 소진되는 검증 행에 감사 정보(직전 호스트명·실행자)를 함께 남긴다.
        $gate = $this->verificationService->consumeForChange($newHost, $domainId, $oldDomain, $actorMemberId);
        if ($gate->isFailure()) {
            return $gate;
        }

        // 저장 (형식·중복 재검증, 구·신 도메인 캐시 무효화, DomainUpdatedEvent 발행)
        $updated = $this->update($domainId, ['domain' => $newHost]);
        if ($updated->isFailure()) {
            return $updated;
        }

        // 옛 도메인의 라우트 캐시 파일은 참조되지 않으므로 직접 정리한다.
        Router::clearRouteCache($oldDomain);

        $this->logger?->channel('domain')->info('도메인 호스트명 변경', [
            'domain_id' => $domainId,
     ...
     * 도메인 중복 여부 (단일 판정 경로)
     *
     * 입력값 그대로와 포트를 뗀 형태를 모두 대조한다. 저장된 값은 포트를 포함할 수 있어
     * (개발환경 localhost:9315 등) 한쪽만 보면 놓친다.
     *
     * 과거에 이 판정이 생성·수정·AJAX 확인 세 곳에 복사돼 있었고, AJAX 확인만 포트를 뗀
     * 형태로 비교해 "이미 있는 도메인"을 사용 가능으로 답했다. 판정은 이 메서드 하나로 모은다.
     *
     * @param int|null $excludeId 자기 자신 제외 (수정 시)
     */
    public function isDomainTaken(string $domain, ?int $excludeId = null): bool
    {
        $candidates = array_unique([
            strtolower(trim($domain)),
            $this->stripPort(strtolower(trim($domain))),
        ]);
        ...
        foreach ($candidates as $candidate) {
            ...
            $exists = $excludeId !== null
                ? $this->domainRepository->existsByDomainExcept($candidate, $excludeId)
                : $this->domainRepository->existsByDomain($candidate);
    ...
    public function checkDomainAvailability(string $domain, ?int $excludeId = null): Result
    {
        ...
        if ($this->isDomainTaken($domain, $excludeId)) {
            return $this->fail('이미 등록된 도메인입니다.');
        }
