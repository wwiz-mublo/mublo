...
사이트를 만들 때마다 관리자, 회원, 권한, 게시판, 페이지 편집을 처음부터 다시 만들고 있다면. 사이트를 넘긴 뒤에도 운영자가 문구 하나 바꾸려고 매번 연락해 온다면. 한 사이트를 위해 고친 코드가 다음 업데이트를 어렵게 만든 적이 있다면 — 머블로는 그런 자리에서 나왔습니다.

Mublo(머블로)는 회원·권한·관리자·페이지 편집 같은 공통 기반을 Core가 맡고, 실제 기능은 필요한 만큼 붙였다 뗄 수 있게 만든 PHP 플랫폼입니다. 하나의 설치본으로 회사 홈페이지, 커뮤니티, 쇼핑몰처럼 성격이 다른 사이트를 도메인별로 나눠 운영할 수 있습니다.

운영자는 코드를 몰라도 블록으로 페이지를 구성하고, 개발자는 자기 기능을 블록으로 공급해 그 편집 화면에 얹습니다. 상품 하나를 메인에 올리는 일이 코드 수정과 배포가 아니라 운영자의 선택이 되는 것 — 그것이 머블로가 블록을 쓰는 이유입니다.

**공식 한글 표기는 "머블로"입니다.** Multi와 Block에서 온 이름입니다.

구조로 말하면, 가벼운 Core 위에 Plugin·Package·Event·Contract·Block 시스템을 얹었습니다. Core는 요청 흐름, 라우팅, DI, 인증, 세션, 렌더링, 이벤트, 관리자 기반처럼 공통 실행 규칙만 담당하고, 실제 기능은 Plugin과 Package로 분리되어 Event 또는 Contract로 느슨하게 연결됩니다.
...
- 사이트를 만들 때마다 관리자·회원·권한·파일·페이지 편집을 다시 만들고 있는 분
- 납품한 사이트를 운영자가 직접 관리하게 하고 싶은 제작자
- 여러 사이트를 하나의 코드베이스로 운영하려는 팀
- 확장 지점을 Event와 Contract로 명시해 장기적으로 관리하려는 팀
- 게시판형 사이트, 브랜드 사이트, 쇼핑몰, 운영형 랜딩 페이지를 한 기반에서 다루려는 팀
...
설치 방법은 두 가지입니다. 파일을 받는 방법만 다르고, 이후 과정은 같습니다.

### 방법 1. 배포 파일 올리기 (명령어 없이)

[릴리스 페이지](https://github.com/wwiz-mublo/mublo/releases/latest)의 배포 zip(`MubloFramework-vX.Y.Z.zip`)에는 의존성(`vendor/`)이 포함되어 있어 Composer가 필요 없습니다. SSH를 쓸 수 없는 공유호스팅에서도 FTP만으로 설치할 수 있습니다.

```text
1. 배포 zip을 받아 압축을 풉니다
2. 파일을 웹 서버에 올리고 public/을 웹 루트로 지정합니다
3. 브라우저에서 https://your-domain.com/install 접속
4. 설치 화면의 안내를 따라갑니다
   환경 점검 → DB 설정 → 도메인 설정 → 보안 설정 → 관리자 계정 생성 → 완료
```

PHP 버전·확장·디렉터리 권한이 맞지 않으면 설치를 진행한 뒤 오류를 만나는 대신, **1단계에서 어느 항목이 부족한지 먼저 알려 줍니다.** 권한은 설정값만 보지 않고 실제로 파일을 생성해 확인하며, 실패하면 디렉터리 소유자·그룹과 PHP 실행 계정을 함께 보여 줍니다.

### 방법 2. 저장소에서 받기 (개발자용)

코드 이력을 Git으로 관리하려면 이쪽을 씁니다.

```bash
git clone https://github.com/wwiz-mublo/mublo.git
cd mublo
composer install
# 이후는 방법 1의 2단계부터 동일합니다
```
