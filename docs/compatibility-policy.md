...
- 기존 이벤트 호환용 `MemberIdentity::getUserId()`의 신규 런타임 호출 금지

이벤트 payload 를 타고 들어가는 의존은 `use` 문이 없어 import 검사에 보이지 않습니다. 그래서 양쪽에서
막습니다.

- **소비자 쪽** — `check-extension-api.php`가 이벤트 클래스를 찾아가 게터의 반환 타입을 읽습니다.
  확장이 `$event->getArticle()`처럼 남의 타입을 꺼내면 실패합니다. 변수에 담아 두든 바로 파고들든
  같습니다 — 게터를 부르는 것 자체가 결합입니다.
- **생산자 쪽** — `check-event-payload.php`가 이벤트 정의를 봅니다. 안정 이벤트가 안정 API 밖의
  타입을 반환하면 실패합니다.

확장이 **자기 소유** 타입을 자기 이벤트로 주고받는 것은 내부 응집이므로 양쪽 모두 통과시킵니다.
소유자가 같으면 한쪽을 리팩터링할 때 다른 쪽도 같이 고치기 때문입니다. 종속 Plugin 에게 부모
Package 의 타입은 남의 타입입니다.
...
마지막 조건은 payload 에도 그대로 적용됩니다. **이벤트 게터는 소유자가 다른 타입을 반환하지
않습니다** — 엔티티를 통째로 넘기면 이벤트가 안정 API 라는 이유로 내부 표면이 확장에 그대로
중계됩니다. 확장이 필요로 하는 값은 스칼라 게터나 Contract DTO 로 넘기고, 커밋된 최신 상태가
필요하면 확장이 해당 Contract 로 재조회하게 합니다.

코어 이벤트의 회원·도메인·블록 페이지 엔티티 게터는 이 규칙에 따라 제거되었습니다. 회원 정보는
`MemberQueryInterface`, 블록 페이지 설정은 `BlockPageRenderingEvent::getPageConfig()` 로 얻습니다.
다시 조회할 수 없는 값(삭제된 도메인 이름, 수정 전 도메인 값)은 이벤트가 스칼라·배열로 실어
보냅니다.
