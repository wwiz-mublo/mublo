...
다만 이 검사는 **확장이 소스에 적은 이름**만 봅니다. 이벤트 게터가 돌려주는 타입을 타고 들어가는
의존(`$event->getMember()->getLevelValue()`)은 `use` 문이 없어 보이지 않습니다. 그 사각지대는
`tools/check-event-payload.php`가 생산자 쪽에서 막습니다 — 안정 이벤트가 안정 API 밖의 타입을
반환하고 있으면 실패합니다. 기존 노출은 `tools/event-payload-baseline.json`에 동결되어 있으며
새 항목을 추가할 수 없습니다.
...
마지막 조건은 payload 에도 그대로 적용됩니다. **이벤트 게터는 안정 API 밖의 타입을 반환하지
않습니다** — 엔티티를 통째로 넘기면 이벤트가 안정 API 라는 이유로 내부 표면이 확장에 그대로
중계됩니다. 확장이 필요로 하는 값은 스칼라 게터나 Contract DTO 로 넘기고, 커밋된 최신 상태가
필요하면 확장이 해당 Contract 로 재조회하게 합니다. `tools/check-event-payload.php` 가 이것을
강제합니다.
