...
### 반쯤 등록된 확장 되돌리기 — ExtensionRegistrationScope

register가 절반쯤 진행되다 boot에서 터진 확장을 그대로 두면, 컨테이너에는 정의가 남고 레지스트리에는 위젯·블록 타입이 등록된 채 "실패한 확장"이 되어 다른 확장의 등록을 계속 가로막는다. `ExtensionRegistrationScope`(`src/Core/Extension/ExtensionRegistrationScope.php`, `@internal`)가 이를 막는다.

- `ExtensionManager`가 확장 하나의 register/boot를 확장 이름(owner) scope로 감싼다(`begin`/`suspend`/`resume`).
- scope가 열려 있는 동안 코어 등록 지점 — `DependencyContainer`, `BlockRegistry`, `DashboardWidgetRegistry`, `EventDispatcher::addListener()` — 이 "이전 상태로 되돌리는 콜백"을 함께 기록한다.
- boot까지 성공하면 `commit()`으로 되돌리기 기록을 버리고, register나 boot에서 실패하면 `rollbackRegistration()`이 그 확장이 남긴 등록만 되감는다. 롤백된 확장은 `loadedExtensions` 목록에서도 제거돼 이후 `isPackageLoaded()` 같은 조회가 "로드됨"으로 오보하지 않는다.
- 롤백 중 발생한 오류도 `stage = 'rollback'`으로 진단에 합류한다.

확장 개발자가 직접 호출할 API는 아니다. 다만 **"실패한 확장은 코어 상태에 흔적을 남기지 않는다"**가 코어가 주는 보장이라는 점은 알아둘 만하다.
