...
- **`name`은 디렉토리가 진실이다.** Manifest에 다른 `name`을 적으면 경고 로그를 남기고 디렉토리명을 사용한다. 로딩·활성 목록·Provider 클래스 조립이 모두 이름 기반이기 때문이다.
...
`packages/Board/Plugins/BoardReport/manifest.json`:
