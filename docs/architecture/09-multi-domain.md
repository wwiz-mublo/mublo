...
| 상태 | 처리 |
|---|---|
| 미등록(`not_found`) · 차단(`blocked`) · 비활성(`inactive`) | `ErrorRenderer`로 에러 페이지 출력 후 종료 |
| `/install`, `/csrf/*`, `/api/v1/csrf/*`, `/serve/*`, 도메인 도달 확인 프로브 | 검증 생략 (설치·공개 API 경로) |

코어는 도메인의 `status`만 본다. 계약 만료·요금제 같은 상용·임대 정책은 해당 Package가 자체 데이터로 판단해 `status`를 바꾸거나 `DomainOwnerValidatingEvent`를 구독해 얹는다 — 코어에 임대 사업 개념을 심지 않는다.
