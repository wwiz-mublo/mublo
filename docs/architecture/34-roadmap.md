...
- **폐기 예정 심볼** — `ConfigFormInterface`(블록 설정 폼, adminScript로 대체됨)와 `SuperOnlyMiddleware`(라우트 억제로 대체됨)는 @deprecated 상태이며 다음 major에서 제거한다.
- **방문 로그 IP 해시 전환** — 현재는 원본 저장 + 자동 퍼지(30일). UV 판정 로직 개편 시 일별 솔트 해시로 전환해 원문 미보관을 검토한다.
