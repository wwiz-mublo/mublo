...
- `getHistory(int $memberId, array $filters = [], int $page = 1, int $perPage = 20): array` — 회원 내역 + 페이지네이션. `$filters` 는 허용 키 동등 조건(domain_id, action, source_type, source_name, reference_type, reference_id)으로 실제 적용된다.
