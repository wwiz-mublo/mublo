...
[17. 블록 시스템](17-block-system.md)이 발행 측을 다룬 블록 페이지 이벤트 4종(`BlockPageCreatedEvent`·`BlockPageUpdatedEvent`·`BlockPageDeletedEvent`·`BlockPageMenuSyncEvent`)의 구독자가 `src/Subscriber/BlockPageMenuSubscriber.php`다(`src/Core/Provider/ServiceProvider.php`에서 등록). 블록 페이지가 생성되면 `MenuService::createItem()`으로 `/p/{code}` 메뉴 항목을 자동 등록하고(`provider_type='core'`, `provider_name='blockpage'`), 코드·제목이 바뀌면 기존 항목을 새 URL·라벨로 동기화하며, 삭제되면 같은 provider의 항목 중 URL이 일치하는 행을 자동 삭제한다. 등록은 menu_items까지이며 menu_tree 배치는 관리자의 수동 작업이다.
...
### 프론트 메뉴 필터 — MenuItemsFilterEvent

`src/Core/Event/Menu/MenuItemsFilterEvent.php`는 프론트 메뉴를 읽는 시점에 확장이 컨텍스트(브랜드샵·파트너 사이트 등)에 따라 아이템을 걸러내는 확장점이다. 저장된 메뉴 구성은 그대로 두고 **이번 요청에서 보일 목록만** 줄인다.

발행 지점은 `MenuService::applyFrontItemsFilter()` 한 곳이며, 네 fetch 메서드가 반환 직전에 통과한다.

| scope 상수 | 발행 메서드 |
|---|---|
| `SCOPE_TREE` | `getTree()` — `getTreeHierarchy()`는 이 결과로 계층을 세우므로 자동 적용 |
| `SCOPE_UTILITY` | `getUtilityMenus()` |
| `SCOPE_FOOTER` | `getFooterMenus()` |
| `SCOPE_MYPAGE` | `getMypageMenus()` |

트리는 **평면 목록 단계**에서 거른다. 계층을 만든 뒤 걸러내면 1차 메뉴만 필터되고 하위 항목이 살아남기 때문이다.

**관리자 요청(`Context::isFront()`가 false)에서는 발행하지 않는다.** 유틸리티·푸터·마이페이지 저장은 "포함 목록 + 순서 전체 교체"라, 표시용 필터로 감춰진 항목이 관리 화면 목록에서 빠진 채 저장되면 그 항목의 노출 설정이 지워진다 — 표시 필터가 데이터를 지우는 결과가 된다. 구독자가 알아서 걸러 주기를 기대하지 않고 코어가 구조적으로 막는다.

구독자는 `getScope()`로 대상을 좁히고 `setItems()`로 줄인 목록을 돌려준다. 아이템 구조는 fetch 메서드에 따라 다르므로(트리는 `menu_code`/`path_code`/`parent_code`, 나머지는 `item_id`) `url` 같은 공통 키만 참조하는 것이 안전하다. 특정 컨텍스트에서만 노출할 메뉴를 감추는 용도로 쓴다 — 예를 들어 서브 컨텍스트에서 특정 경로로 시작하는 메뉴를 전부 걸러낸다.

블록 시스템의 `menu` 콘텐츠 타입은 이 필터를 받지 않는다. `MenuRenderer`가 `MenuService`가 아니라 `MenuTreeRepository`를 직접 읽고, 블록 렌더 결과는 행 단위로 캐시되기 때문이다.
