...
- **BlockRow** — 가로 행. 페이지 소속이거나 위치(position) 소속이다. 유효 위치는 `BlockRowService::VALID_POSITIONS` = `index, left, right, subhead, subfoot, contenthead, contentfoot` (`src/Service/Block/BlockRowService.php`)
...
- **`rendererClass` 필수** — `src/Core/Block/Renderer/RendererInterface.php`(`render(BlockColumn): string`)를 구현해야 한다. 등록 시 `is_subclass_of` 검증, `createRenderer()` 시점에 재검증한다.
...
- **중복 등록은 예외** — 같은 `type`을 다시 등록하면 `InvalidArgumentException`. 의도적 교체만 `options: ['allowOverwrite' => true]`로 허용된다.
