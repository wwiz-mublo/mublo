...
정의된 필드의 렌더·검증·파일 처리는 `src/Service/CustomField/`의 공용 3종이 담당한다. "공용"이 요점이다 — 회원 필드만이 아니라 주문 필드 등 모든 커스텀 필드 시스템이 같은 클래스를 쓴다. 같은 디렉터리의 `CustomFieldValueValidator`·`CustomFieldFileManager`는 이 셋을 `Mublo\Contract\CustomField\*` 계약 뒤에 감싼 어댑터로, 확장은 정적 호출 대신 이 계약을 주입받는다([16장](16-contract-catalog.md)).
...
- `docs/reference/event-reference.md` — 이벤트 구독·발행 코드와 페이로드 확인법
