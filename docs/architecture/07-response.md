...
모든 Response의 공통 부모는 `src/Core/Response/AbstractResponse.php`의 `AbstractResponse`이며, 하위 타입은 여섯 개다(모두 `src/Core/Response/` 아래).
...
| `XmlResponse` | sitemap.xml·RSS 등 XML 문자열 응답 |
...
출력 단계에서 `Application::handleResponse()`는 타입을 순서대로 검사한다. `ViewResponse`는 Renderer에 위임하고, `JsonResponse`/`RedirectResponse`는 상태 코드와 헤더를 직접 전송하며, `HtmlResponse`/`FileResponse`는 각자의 `send()`를 호출한다. 명시 분기에 없는 타입이라도 `send()` 메서드를 가진 객체면 그것을 호출한다 — `XmlResponse`가 이 경로로 출력되며, Package가 커스텀 Response를 정의할 수 있는 확장점이기도 하다. `send()`조차 없으면 `RuntimeException`이다.
...
```php
// PG 콜백 등 단순 텍스트 — src/Core/Response/HtmlResponse.php 주석의 사용 예
return new HtmlResponse('SUCCESS');
return new HtmlResponse('FAIL', 403);
```

XML은 `HtmlResponse`에 헤더를 얹는 대신 `XmlResponse`를 쓴다(아래).
...
public function __construct(
    ...
    bool $deleteAfterSend = false  // 전송 후 원본 삭제 (임시 리포트 파일 등)
)
```
...
`src/Core/Response/XmlResponse.php`. 생성자는 `__construct(string $xml, int $statusCode = 200)`이고 `Content-Type: application/xml; charset=UTF-8`을 스스로 설정한다. 조회용 `getXml()`과 출력용 `send()`를 가진다. Core에서는 `src/Controller/Front/SitemapController.php`가 사용처다.

```php
// src/Controller/Front/SitemapController.php — 발췌
return new XmlResponse($this->renderXml($urls));
```

## 경계

Response 클래스 여섯 종과 `AbstractResponse`는 Controller가 직접 생성하는 공개 표면이다. 반면 `ViewResponse`의 Renderer 전용 getter나 `Application::handleResponse()`의 분기 순서는 내부 구현이다. Core 안정 API의 최종 목록은 `docs/compatibility-policy.md`가 진실이다.
