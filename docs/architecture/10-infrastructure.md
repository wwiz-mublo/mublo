...
디렉토리는 14개다: `AI`, `Cache`, `Code`, `Cookie`, `Crypto`, `Database`, `Image`, `Log`, `Mail`, `Maintenance`, `Redis`, `Security`, `Session`, `Storage`. 이 중 `AI`(`src/Infrastructure/AI/AiHttpClient.php` — AI 제공자 HTTP 호출 전용 cURL 클라이언트)는 AI 시스템의 내부 부품이므로 여기서는 존재만 언급하고 [27. AI 시스템](27-ai.md)에서 다룬다.
...
`increment(string $key, int $value = 1, ?int $ttl = null): int|false` 는 원자적 증가를 드라이버에 맡긴다. `get()` 후 `set()` 하는 방식은 동시 요청에서 증가분이 유실되므로, 카운트가 곧 방어선인 곳(레이트 리밋)에서는 쓰면 안 된다. `$ttl` 은 키가 새로 만들어질 때만 적용된다 — 증가할 때마다 만료를 갱신하면 요청이 계속 들어오는 한 윈도우가 끝나지 않는다. `RedisCache` 는 `SET NX EX` + `INCRBY` 로, `FileCache` 는 `flock` 임계 구역으로 보장한다.
...
`Logger`(`src/Infrastructure/Log/Logger.php`)는 파일 기반 로거이며 `Psr\Log\LoggerInterface` 를 구현한다. 컨테이너는 구체 클래스와 인터페이스 양쪽으로 같은 인스턴스를 내주므로, 확장은 `LoggerInterface` 에 의존하면 된다. 운영자가 다른 구현으로 갈아끼울 때는 `Application` 의 등록만 바꾼다. `storage/logs/D{domainId}/{channel}/{날짜}.log` 구조로 도메인별·채널별·일별로 분리되며, 도메인 미설정 시 `_system/`에 쓴다. `channel('payment')->info(...)`처럼 채널을 전환해 확장 전용 로그 파일을 만들 수 있다. `debug`/`info`/`warning`/`error`/`critical` 레벨과 최소 레벨 필터링을 지원한다. 결제·외부 API 연동처럼 사후 추적이 필요한 확장이라면 반드시 주입받아야 할 서비스다.

코어에는 `error_log()` 호출도 남아 있는데 이는 정리되지 않은 잔재가 아니라 폴백 경로다. 부팅 중이라 컨테이너가 아직 없는 지점(`ExtensionManager`, Provider 의 확장 등록), 로거가 주입되지 않은 지점(`ErrorHandler`, `EventDispatcher` 의 null 분기), 그리고 로거 자신이 실패한 지점(`Logger` 의 파일 쓰기 실패, 로거 호출이 예외를 던진 경우)이다. 이 자리들을 `Logger` 로 바꾸면 로거 장애를 기록할 수단이 사라지므로 그대로 둔다.

`bootstrap.php` 가 `error_log` 목적지를 `storage/logs/php-error.log` 로 고정하므로 두 갈래가 같은 디렉터리에 모이고, `DailyStorageCleanup` 의 보존기간 정리에도 함께 걸린다. 확장이 새로 로깅을 추가할 때는 `error_log()` 가 아니라 `LoggerInterface` 를 주입받는다 — 위의 세 경우가 아니라면 폴백을 쓸 이유가 없다.

### Maintenance

`DailyStorageCleanup`(`src/Infrastructure/Maintenance/DailyStorageCleanup.php`)은 별도 스케줄러(cron)가 없는 설치 환경을 위한 기회주의적 정리기다. 한국 시간 03:00~05:00 사이의 첫 웹 요청 한 번만 오래된 파일 세션과 로그를 지우고, 잠금 파일로 동시 실행을 막으며, 한 번에 타입별 최대 삭제 건수를 제한한다(기본 보존 7일). 그 시간대에 요청이 없던 날은 그냥 건너뛴다 — 정리를 위해 요청을 지연시키지 않는 것이 설계 의도다.
...
두 계층이 결합된 실례가 에디터 이미지 업로드 API다. `POST /api/v1/editor/upload`(`src/Core/App/Router.php`에 등록, 처리는 `src/Controller/Api/EditorUploadController.php`)는 CSRF 미들웨어를 경유하므로 `X-CSRF-Token` 헤더가 필요하고, 이미지 확장자만 허용(UploadPolicy 안에서 좁히기)하며, 비인증 접근이 가능하므로 `RateLimiter`로 IP당 10분에 60회로 제한한다.

크기 상한은 `UploadConfig`(`src/Core/UploadConfig.php`)가 `config/upload.php`에서 읽어 로그인 회원(기본 20MB)과 비로그인 방문자(기본 5MB)에 다르게 적용한다. 둘을 나눈 이유는 이 경로가 인증 없이 호출되고 저장된 파일이 공개 URL로 열리기 때문이다 — 회원과 같은 한도를 주면 무단 업로드로 디스크를 채우거나 파일 호스팅으로 악용할 여지가 그만큼 커진다. 레이트 리미터는 횟수만 막으므로 크기 상한을 대체하지 못한다. 클라이언트(MubloEditor, MubloRequest)는 크기를 검사하지 않는다. 허용 크기는 이 설정과 php.ini가 함께 정하는 값이고 브라우저는 그중 어느 것도 알지 못하므로, 임의의 상한을 두면 서버가 허용하는 파일까지 전송 전에 막힌다.
