...
- 반환 타입은 `@template` 으로 인자와 묶여 있어 넣은 이벤트의 구체 타입이 그대로 나온다. 서비스마다 두는 private `dispatch()` 래퍼도 같은 애노테이션을 붙여야 호출부에서 타입이 살아 있다 — 빠뜨리면 정적분석이 구독자가 채운 값을 읽는 자리를 "없는 메서드"로 본다.
- 생성자는 `__construct(?callable $exceptionLogger = null, bool $rethrowListenerFailures = false)`다. 리스너에서 일반 예외(`\Throwable`)가 발생하면 기본적으로 이 로거에 기록하고 다음 리스너를 계속 실행한다(best-effort). 단 `\Error`(TypeError 등 치명 오류)는 항상 다시 던진다.
- `$rethrowListenerFailures`는 삼킨 예외를 다시 던질지 정하는 개발용 스위치다. `ServiceProvider`가 `APP_ENV !== 'production'`일 때 `true`로 등록한다 — 운영에서는 구독자 하나가 페이지 전체를 죽이면 안 되지만, 개발에서 조용히 삼켜진 예외는 기능이 사라진 줄도 모르게 만들기 때문이다.
...
메서드가 없는 마커 인터페이스다. 기본 정책(리스너 예외를 기록하고 계속 진행)이 적합하지 않은 이벤트 — 리스너 실패가 데이터 정합성 문제로 이어지는 것 — 이 이 마커를 구현하면, `EventDispatcher::dispatch()`가 리스너의 일반 예외를 삼키지 않고 호출자에게 다시 던진다(`EventDispatcher.php`의 `$event instanceof FailFastEventInterface` 분기). UI 장식이나 추적처럼 일부 실패해도 요청을 계속해야 하는 이벤트는 구현하지 않는다.

기준은 단순하다 — **차단·검증이 가능한 사전(-ing) 이벤트와 트랜잭션 안에서 발행되는 이벤트는 fail-fast여야 한다**. 구독자의 예외가 조용히 삼켜지면 "검증에 통과한 것"과 구분되지 않기 때문이다. 현재 구현체는 다음과 같다.

- Core: `BalanceAdjustingEvent`, `MemberRegisterValidatingEvent`, `MemberRegisterPreparingEvent`, `DomainOwnerValidatingEvent`, `DomainProvisioningEvent`(도메인 생성 트랜잭션 내 필수 초기화), `MemberDeletingEvent`·`MemberWithdrawingEvent`(`src/Service/Member/Event/`)
- Board Package: `ArticleCreatingEvent`, `ArticleUpdatingEvent`, `ArticleDeletingEvent`, `ArticleViewingEvent`, `CommentCreatingEvent`, `FileDownloadingEvent` — 이 규약은 `packages/Board/tests/Unit/Event/BlockableEventsFailFastTest.php`가 기계적으로 검사한다.
...
부모 Package인 Board도 같은 패턴으로 `packages/Board/BoardProvider.php`의 `boot()`에서 메뉴 자동등록·검색·관리자 메뉴·블록 캐시·도메인·포인트·반응 포인트·알림·마이페이지·블록 아이템·페이지 타입 등 구독자 11종을 `addSubscriber()`로 등록한다.
...
| Block | `BlockPageUpdatedEvent` | 블록 페이지 코드·제목 변경 후 자동 메뉴 동기화 |
| Block | `BlockPageMenuSyncEvent` | 대체 생성 경로에서 블록 페이지 메뉴 아이템 동기화 요청 |
| Block | `BlockPageRenderingEvent` | 블록 페이지(`/p/코드`) 렌더링 시 추가 HTML 주입 |
| Domain | `DomainCreatedEvent` | 도메인 생성 후(커밋 이후) 선택적 초기 데이터 시딩 |
| Domain | `DomainProvisioningEvent` | 도메인 생성 **트랜잭션 안**의 필수 초기화 — fail-fast(실패 시 전체 롤백) |
| Domain | `DomainOwnerValidatingEvent` | 코어 기본 소유자 검증 후 확장 정책 추가 검증 — `addError()`, fail-fast |
...
| Menu | `MenuItemsFilterEvent` | 프론트 메뉴 fetch 시 컨텍스트에 따라 아이템 제외 (scope: tree/utility/footer/mypage). 관리자 요청에서는 발행하지 않는다 — [25장](25-search-mypage-menu.md) |
...
| Rendering | `FrameTemplateSourceCollectEvent` | 도메인 프레임 편집 템플릿의 치환 변수·컴포넌트 슬롯 수집(`확장명.이름` 네임스페이스 강제) |
...
레퍼런스인 Board Package는 `packages/Board/Event/`에 20개의 공식 Event를 둔다.
...
| 댓글 | `CommentCreatingEvent`(차단 가능) / `CommentCreatedEvent`, `CommentUpdatedEvent`, `CommentDeletedEvent` |
