...
| Shop | 상품·장바구니·주문·결제 쇼핑몰 패키지 | 결제 Contract 소비([16](16-contract-catalog.md)), 카테고리 공급, 알림 변수 구독, 전환 추적([26](26-tracking.md)), Report 정의 등록([23](23-report-engine.md)) |
...
| Faq | FAQ 관리 | `FaqQueryInterface` 1:1 바인딩 + `FaqProvisioningInterface`([16](16-contract-catalog.md)) |
| Popup | 레이어 팝업 관리 | 데이터 리셋 계약 구현 |
| Qna | 질문과 답변 관리 | 데이터 리셋 계약 구현 |
| Survey | 설문 생성·참여·집계 | 블록 시스템 연동 |
| Manual | 매뉴얼 책·페이지 관리(트리 목차 + 프론트 열람) | `ManualQueryInterface` 1:1 바인딩([16](16-contract-catalog.md)) |
...
| EmailNotify | 이메일 템플릿·이미지·발송 이력 관리 | `EmailTemplateProviderInterface` 구현 — 전송로는 코어 게이트웨이(24장) |
| SendonSms | 센드온 SMS/LMS/MMS 발송 | `NotificationGatewayInterface` 구현(24장) |
| SendonTalk | 센드온 카카오 알림톡 발송 | 〃 |
| SnsLogin | 네이버·카카오·Google 소셜 로그인 | 가입 이벤트 체인 연동(19장), `MemberAuthenticatorInterface` 소비 |
| MemberPoint | 회원 포인트 적립·차감·내역 | Balance 계약·이벤트 연동(21장) |
