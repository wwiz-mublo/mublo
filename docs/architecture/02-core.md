...
3. **도메인 유효성 검증** — `validateDomain()`. 미등록(`not_found`)·차단(`blocked`)·계약 만료(`expired`)·비활성(`inactive`) 도메인이면 `ErrorRenderer`로 에러 페이지를 출력하고 종료한다. `/install`과 일부 공개 API(`/csrf/`, `/serve/`)는 검증을 건너뛴다.
...
6. **전역 Middleware 실행** — `MiddlewarePipeline`에 `SessionMiddleware`, `CsrfMiddleware`(모두 `src/Core/Middleware/`)를 태워 실행한다. 파이프라인의 최종 핸들러 안에서:
