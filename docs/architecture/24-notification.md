...
Core는 내부 회원 알림을 직접 저장하지만 외부 채널을 자동 발송하지 않는다. 외부 발송 쪽에서 Core가 가진 것은 계약(`src/Contract/Notification/`), `ContractRegistry`(1:N 등록·메타 스키마 검증, [16장](16-contract-catalog.md)), 그리고 어디에도 자동 등록되지 않는 구현체 하나(`src/Core/Notification/EmailNotificationGateway.php`)다. 발송 큐, 재시도, 채널 통합 발송 이력 같은 코어 수준 파이프라인은 현재 없다 — 발송 이력은 각 알림 Plugin이 자기 로그 테이블에 기록한다(`plugins/SendonTalk/Repository/SendonTalkLogRepository.php` 등).
...
| Plugin | 등록 키 | 채널 메타 | 실발송 경로 |
|---|---|---|---|
| `plugins/EmailNotify/` | `email_notify` | `['email']` | 코어 `Mailer` 기반 `EmailNotifyService` |
| `plugins/SendonSms/` | `sendon_sms` | `['sms']` | 센드온 API(`Service/SendonApiClient.php`) |
| `plugins/SendonTalk/` | `sendon_talk` | `['alimtalk']` | 센드온 API + 카카오 템플릿 검수 연동 |

등록은 세 Plugin 모두 같은 형태다.
...
세 Plugin의 내부 구조도 동형이다. 새 채널 Plugin을 만들 때 이 구조를 그대로 참조하면 된다.
...
### Core EmailNotificationGateway는 왜 자동 등록되지 않는가

`src/Core/Notification/EmailNotificationGateway.php`는 코어 `Mailer`로 이메일을 보내는 완성된 구현체지만, `src/Core/Provider/ServiceProvider.php`의 ContractRegistry 팩토리는 의도적으로 등록하지 않는다.

```php
// src/Core/Provider/ServiceProvider.php — ContractRegistry 팩토리 말미
// 이메일은 코어 기본 채널로 등록하지 않는다.
// 전용 이메일 게이트웨이 플러그인이 register()로 등록할 때만 노출된다.
```

채널 공급은 예외 없이 Plugin의 결정 사항이라는 경계다. 코어가 `email` 채널을 선점하면 템플릿 CRUD·발송 이력을 갖춘 전용 Plugin(EmailNotify)과 같은 채널을 두 게이트웨이가 나눠 갖게 되고, 운영자가 관리 화면에서 통제할 수 없는 발송 경로가 생긴다. 결과적으로 이 저장소의 실제 `email` 채널 공급자는 EmailNotify Plugin이며, Core 구현체는 "Plugin이 원하면 재사용할 수 있는 부품"으로만 존재한다.
