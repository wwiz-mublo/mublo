...
- 확장(plugins/, packages/) 소스의 `use` 문을 수집해 안정 API 목록과 대조한다. import 없이 완전한 클래스명(FQCN)으로 우회하는 경우도 `token_get_all()`로 함께 잡는다(문자열·주석 속 예시는 제외).
- 확장이 소유한 테스트 코드는 검사 대상이 아니다 — 배포되는 런타임 코드가 아니고, 픽스처가 코어 Entity를 직접 쓰는 것은 정상이다.
- 독립 Plugin 간 참조(`Mublo\Plugin\*`)는 requires의 소관이므로 검사하지 않는다.
...
- 이름이 `Event`로 끝나거나 `\Event\` 아래에 있는 심볼은 위치와 무관하게 안정으로 본다(코어가 `Service\Admin\Event\` 처럼 `Core\Event\` 밖에도 이벤트를 두기 때문이다).

번들 확장의 운영 코드 baseline은 **0건**이다. 새 비안정 의존이 들어오면 CI가 즉시 실패한다.

### 문서와 도구가 어긋나면 도구가 진실이다

`docs/compatibility-policy.md`는 안정 API를 사람이 읽는 형태로 선언하고, `tools/check-extension-api.php`의 `$stablePrefixes` 배열이 그것을 코드로 옮긴 것이다. 스크립트 주석이 못을 박는다 — **두 목록이 어긋나면 문서가 아니라 이 배열이 진실이다. CI가 강제하는 것은 배열이기 때문이다.** 안정 API를 넓히거나 좁힐 때는 문서와 배열을 같은 커밋에서 함께 고친다.
