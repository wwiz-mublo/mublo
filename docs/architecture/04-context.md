...
| 도메인 | `getDomain()`, `getDomainInfo()`, `hasDomainInfo()`, `isDomainAccessible()`, `getDomainId()`, `getDomainGroup()` |
| 스킨 | `getAdminSkin()`, `getFrameSkin()`, `getFrameBasePath()`, `getFrontSkin($group)`, `getBlockSkin($type)` |
| 메뉴 | `getCurrentMenuCode()` |
...
- 도메인 유효성 검증(미등록·차단·만료·비활성)도 Context의 `getDomainInfo()`를 읽어 수행된다.
