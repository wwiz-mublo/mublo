...
두 렌더러는 `ViewRendererInterface`(`src/Core/Rendering/ViewRendererInterface.php`)를 구현하며, `render(ViewResponse $response, Context $context): void` 하나가 계약의 전부다. 조립의 주도권은 항상 렌더러에 있고 `LayoutManager`는 도구다. 이 외에 프레임 밖 에러 화면(도메인 미등록·차단·비활성, 404/403/500)은 `src/Core/Rendering/ErrorRenderer.php`가 독립적으로 담당한다.
...
Admin 셸(`views/Admin/frame/basic/`)은 `LayoutClose.php` 대신 `Sidebar.php`를 가진다. `_assets`는 웹루트 밖이므로 `ServeController`가 서빙한다(`src/Controller/Api/ServeController.php`). 스킨명은 `[A-Za-z0-9_-]`만 허용하고 그 외에는 400이다.

| URL | 실제 경로 |
|---|---|
| `/serve/front/{skin}/{path}` | `views/Front/frame/{skin}/_assets/` |
| `/serve/front/view/{group}/{skin}/{path}` | `views/Front/{Group}/{skin}/_assets/` — 콘텐츠 스킨 에셋 |
| `/serve/admin/{skin}/{path}` | `views/Admin/frame/{skin}/_assets/` |
| `/serve/package/...`, `/serve/plugin/...` | 확장 소유 에셋 (부모 비활성·실패 시 차단) |
...
도메인 운영자가 블록 에디터에서 header/footer를 HTML로 직접 편집(수동 + AI)해 파일 스킨 대신 렌더하는 기능. 규격의 진실은 코드다 — 렌더 배선은 `src/Core/Rendering/FrontViewRenderer.php`와 `FrameTemplateRenderer`, 편집·게시 API는 `src/Controller/Admin/BlockEditorController.php`, 템플릿 소스 목록은 `src/Core/Rendering/CoreFrameTemplateSources.php`가 소유한다.
