...
- **코어가 하지 않는 것**: 방문 데이터 저장, 집계, 통계 화면, 봇 판별, 캠페인 키 관리. 코어에는 통계 테이블이 하나도 없다 — 통계 테이블 7종은 모두 VisitorStats의 마이그레이션이 만든다.
...
테이블은 7개다(`plugins/VisitorStats/database/migrations/001_create_visitor_stats_tables.sql`): 원본 로그 `plugin_visitor_logs`, 집계 `plugin_visitor_daily`/`_hourly`/`_pages`/`_referrers`/`_campaigns`, 캠페인 키 설정 `plugin_visitor_campaign_keys`. 페이지 URL은 쿼리스트링·프래그먼트를 제거하고 저장한다(`cleanUrl()`).
...
VisitorStats의 전환 통계 화면은 `form_submissions` 테이블(AutoForm 확장의 폼 접수 데이터)을 읽기 전용으로 참조해 캠페인별 전환율을 계산한다(`plugins/VisitorStats/Repository/ConversionRepository.php`). 테이블이 없으면 `isAvailable()`이 false를 반환해 화면이 빈 상태로 동작한다(`plugins/VisitorStats/Service/ConversionStatsService.php`). 즉 전환 화면은 AutoForm 설치를 전제로 하는 소프트 의존이다.
...
| `TrackingKeys` | 공개 Contract (16장 카탈로그 소속) |
| `MubloTracking.js` | 공개 클라이언트 라이브러리 (28장 소속) |
| `VisitorCollector`, `VisitorStatsService`, Repository 7종 | VisitorStats 내부 구현 — 외부 참조 금지 |
