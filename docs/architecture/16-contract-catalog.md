...
[15. Public API](15-public-api.md)가 "무엇이 안정 API인가"라는 경계 원칙을 다뤘다면, 이 장은 그 경계 위에 실제로 존재하는 Core 공개 계약(`Mublo\Contract\*`)의 카탈로그다. 확장이 자주 마주치는 계약은 절을 나눠 자세히, 나머지는 뒤쪽 표에 도메인별로 빠짐없이 싣는다.
...
| 계약 | 모드 | 구현 |
|---|---|---|
| `NotificationGatewayInterface` | 1:N ContractRegistry | `src/Core/Notification/EmailNotificationGateway.php`, `plugins/SendonSms/SendonSmsGateway.php`, `plugins/SendonTalk/SendonTalkGateway.php` |
...
소비 사례는 `packages/Shop/Action/NotificationActionHandler.php`(주문 이벤트에 따라 키로 게이트웨이를 선택해 발송)다.

이메일 채널은 **역할이 둘로 갈린다.** 전송로는 Core 의 `EmailNotificationGateway` 가 `core_email` 키로 **항상 등록**하고(`src/Core/Provider/ServiceProvider.php`), 템플릿(내용물)은 `EmailTemplateProviderInterface` 공급자가 댄다 — EmailNotify 플러그인이 그 공급자다. 즉 EmailNotify 는 이 계약의 구현체가 아니라 소비자다.

과거에는 전용 플러그인이 게이트웨이까지 등록했는데, 관리자 채널 트리에는 이메일이 보이면서 실제 발송은 게이트웨이 부재로 실패하는 어긋남이 있어 전송로를 코어로 올렸다.
...
### 코어가 구현하고 확장이 주입받는 계약

위 계약들이 대부분 "확장이 구현하고 코어·다른 확장이 소비"하는 방향이라면, `src/Contract/` 에는 반대 방향 — **코어가 단일 구현을 제공하고 확장은 인터페이스로 주입받기만 하는** 계약이 한 무리 더 있다. 확장이 코어 내부 Service·Repository·Entity를 직접 붙잡지 않게 만드는 것이 목적이며, 바인딩은 전부 `src/Core/Provider/ServiceProvider.php`의 컨테이너 싱글톤이다.
...
| CustomField | `CustomFieldValueValidatorInterface`, `CustomFieldFileManagerInterface` | 커스텀 필드 검증·정규화와 파일 타입 2단계 업로드 처리 |
| Block | `BlockKitGatewayInterface`, `BlockPageQueryInterface` | 블록 킷 공급·블록 페이지 조회 ([17장](17-block-system.md)) |
| Block | `BlockPreviewRendererInterface`, `BlockColumnView`, `BlockRenderContextInterface` | 저장 전 구성 미리보기 렌더·칸 뷰 계약 |
| Block | `BlockContentCacheInvalidatorInterface` | 콘텐츠 타입·아이템 단위 블록 캐시 무효화 |
| Site | `CompanyInfoInterface`, `DomainQueryInterface`, `DomainDescriptor` | 회사 정보·도메인 정보 조회 |
| Security | `CsrfTokenProviderInterface` | 폼·AJAX용 CSRF 토큰 발급 |
| Security | `SensitiveValueCodecInterface` | 민감 값 암호화 + 검색 인덱스(암호화된 값의 동등 검색) |
...
시작 킷·설치 마법사처럼 "사이트 구성을 프로그래밍으로 구축"하는 코드를 위한 계약군이다. 모두 **멱등 생성**이 규약이며, 코어가 정의하고 해당 기능을 소유한 확장이 구현한다.

| 계약 | 구현 | 만드는 것 |
|---|---|---|
| `Site/SiteProvisioningInterface` | 코어 | 메뉴·페이지 등 사이트 골격 |
| `Site/ManagedSiteGatewayInterface`, `ManagedSite` | 코어 | 하위 사이트 생성과 소유자 준비 |
| `Menu/MenuManagementInterface`, `MenuDescriptor` | 코어 | 이미 만들어진 확장 메뉴의 조회·수정·삭제(생성은 SiteProvisioning) |
| `Board/BoardProvisioningInterface` | Board Package | 게시판 |
| `Catalog/CatalogProvisioningInterface` | 카탈로그 확장 | 항목 카탈로그 |
| `Faq/FaqProvisioningInterface` | Faq Plugin | FAQ 카테고리 |
| `Form/FormProvisioningInterface` | AutoForm Plugin | 문의·신청 폼 |

### 나머지 조회·수집 계약

| 계약 | 모드 | 구현 → 소비 |
|---|---|---|
| `Manual/ManualQueryInterface`(+`ManualBook`·`ManualPageNode`·`ManualPageDetail`) | 1:1 | Manual Plugin → 코어·Package |
| `Promotion/PromotionQueryInterface` | 1:1 | Promotion Plugin → 블록·다른 Package 스킨 |
| `Sitemap/SitemapUrlProviderInterface` | 1:N | Package/Plugin → 코어 `SitemapController` |
| `Tracking/ConversionRecordedEvent`, `ConversionSourceTypes` | Event·상수 | 전환 기록 통지와 출처 타입 상수 ([26장](26-tracking.md)) |
| `Notification/NotificationChannelTreeBuilderInterface`, `NotificationTemplateContextInterface`, `EmailTemplateProviderInterface` | 컨테이너 1:1 | 코어 → 알림 Plugin·Package |

### 요약표

| 도메인 | 계약 | 모드 | 번들 구현 | 주 소비자 |
|---|---|---|---|---|
| 결제 | `PaymentGatewayInterface` | 1:N | PayApp, TossPay, IniPay, Kcp, NicePay, TestPay | Shop |
| 결제(반대 방향) | `PaymentConsumerInterface` | 1:N | 주문을 가진 Package | 결제 Plugin의 콜백 |
...
| 추적 | `TrackingKeys`, `ConversionSourceTypes`, `ConversionRecordedEvent` | 상수·Event | — | VisitorStats ↔ Shop |
| 코어 제공 계약 | Auth·Member·CustomField·Block·Site·Security·Extension·Frame 등 | 컨테이너 1:1 | Core | 모든 확장 |
| Provisioning | `SiteProvisioningInterface` 외 6종 | 컨테이너 1:1 | Core·기능 소유 확장 | 시작 킷·설치 마법사 |
| 조회 | `ManualQueryInterface`, `PromotionQueryInterface` | 1:1 | Manual, Promotion Plugin | 코어·Package 스킨 |
| 사이트맵 | `SitemapUrlProviderInterface` | 1:N | Package/Plugin | 코어 `SitemapController` |
