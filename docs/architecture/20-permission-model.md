...
### SuperOnlyMiddleware — 최고관리자 한정 보조 가드

`src/Core/Middleware/SuperOnlyMiddleware.php`. `isSuper()`만 통과시킨다. 로그인·관리자 검증은 하지 않으므로 **반드시 AdminMiddleware 뒤에 체인**해야 한다(클래스 주석 원문). manifest의 `super_only: true` 플러그인 admin 라우트가 적용 예로 명시돼 있으나, 현재 코어의 `super_only` 강제는 미들웨어가 아니라 라우트 등록 억제로 구현돼 있다 — `Router::isSuperOnlyPluginOnSubSite()`가 하위 도메인에서 해당 플러그인의 라우트 자체를 등록하지 않는다([09. 멀티 도메인](09-multi-domain.md)).
...
반대로 autoResolve는 SuperOnlyMiddleware를 붙여 주지 않는다. 자동 매핑되는 관리자 컨트롤러에서 최고관리자·도메인 운영자 제한이 필요하면 메서드 안에서 직접 확인해야 한다(아래 확장 규약). `/admin/admin-permissions` 화면이 실례다 — autoResolve로 매핑되고, 컨트롤러가 `isSuper()`를 직접 확인한다.
...
**Anti Pattern** — 프론트 응답에서 `is_admin` 같은 세션 값을 뷰 분기에만 쓰고 서버 판정을 생략하는 것, 관리자 쓰기 엔드포인트 URL에 액션 키워드를 넣지 않아 `list`로 오판되게 만드는 것, SuperOnlyMiddleware를 AdminMiddleware 없이 단독으로 붙이는 것.
