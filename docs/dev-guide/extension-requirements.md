...
`options.capabilities` 로 선언한 능력만 UI 로 노출한다. 선언을 빠뜨리면 기능이
있어도 운영자에게 보이지 않는다.

- **MUST** — 신규 타입은 `BlockRegistry::capabilities()`로 `skin`, `items`,
  `count`, `style`, `aos`, `customConfig` 여섯 값을 모두 선언한다.
- **MUST** — 각 값은 렌더러와 관리자 스크립트가 실제 소비하는 기능만 켠다.
- **MUST** — `items`와 `count`, `items`와 `customConfig`를 같은 의미로 취급하지 않는다.
- **SHOULD** — `hasItems`와 `hasStyle`은 구 소비자 호환이 필요한 동안만 함께
  선언한다. 신규 관리자 UI의 판정 기준은 정규화된 capability다.

### 전용 편집 adapter

- **MUST** — 공통 capability로 표현할 수 없는 전용 UI는 `options.editor`에
  adapter 이름과 `inline`/`modal` 진입 방식을 선언한다.
- **MUST** — 확장 adapter는 `adminScript`에서
  `MubloBlockEditorAdapters.register()`로 필요한 생명주기 훅을 등록한다.
- **MUST NOT** — 콘텐츠 타입을 추가하기 위해 코어 `blockrow-form.js`에 타입명
  또는 adapter명 조건문을 추가하지 않는다.
- **SHOULD** — 객체형 `content_items`를 직접 소유할 때만 `ownsItems: true`와
  `loadItems`/`collectItems`를 함께 구현한다.
