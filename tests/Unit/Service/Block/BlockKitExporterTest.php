...
class BlockKitExporterTest extends TestCase
{
     ...
     * 확장 타입을 등록한다.
     *
     * 킷 이식 정책은 타입 소유자가 선언하므로(options['kitPortable']), 미등록 타입은
     * 물어볼 상대가 없어 아이템을 담지 않는다. 확장 타입을 다루는 테스트는 그 확장이
     * 부팅된 상태를 만들어야 실제 동작과 같아진다.
     */
    private function registerExtensionType(string $type, bool $kitPortable = true): void
    {
        BlockRegistry::registerContentType(
            $type,
            'PACKAGE',
            $type,
            'Mublo\\Tests\\Fake\\Renderer',
            null,
            ['allowOverwrite' => true, 'skipValidation' => true, 'kitPortable' => $kitPortable]
        );
    }
    ...
    {
        $this->registerExtensionType('board');
    ...
    /**
     * 이식 불가로 선언된 타입은 저작자가 참조 포함을 체크해도, clone 모드여도 비운다.
     * 배너 아이템은 이 도메인에 업로드된 이미지를 가리켜 옮겨 봐야 깨지기 때문이다.
     */
    public function testExportRowsAlwaysDropsNonPortableContentItems(): void
    {
        $this->registerExtensionType('banner', kitPortable: false);
    ...
    /**
     * 확장이 비활성이면 그 타입의 칸은 DB 에 남아 있어도 이식 가능 여부를 물어볼
     * 상대가 없다. 킷은 다른 설치로 옮겨지므로 모르면 담지 않는다 —
     * kit_needs_items 로 표시되어 가져오기 쪽에서 다시 지정할 수 있다.
     */
    public function testUnregisteredTypeDropsContentItemsAndFlagsThem(): void
    {
        $row = BlockRow::fromArray(['row_id' => 10, 'domain_id' => 1, 'position' => 'index']);
        $column = BlockColumn::fromArray([
            'column_id' => 99,
            ...
            'domain_id' => 1,
            'column_index' => 0,
            'content_type' => 'type_from_disabled_extension',
            'content_kind' => 'PACKAGE',
            'content_items' => [['id' => 7]],
        ]);
