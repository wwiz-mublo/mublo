...
class RouterAutoResolveTest extends TestCase
{
    ...
    /**
     * PHP 는 클래스·메서드 이름의 대소문자를 가리지 않아 /admin/MEMBER/delete 도
     * MemberController::delete 로 해석됐다. 그런데 관리자 권한 검사는 요청 경로를
     * 메뉴 URL 과 대소문자까지 그대로 비교하므로 이런 경로는 어떤 메뉴에도 매칭되지
     * 않고, 빈 메뉴코드는 isDenied() 가 무조건 허용한다. 차단된 메뉴의 상태 변경
     * action 에 대소문자만 바꿔 닿을 수 있었다.
     */
    #[DataProvider('miscasedAdminPathProvider')]
    public function testMiscasedAdminPathIsRejected(string $path): void
    {
        $this->expectException(HttpNotFoundException::class);
        $this->resolve($path, true, 'POST');
    }

    public static function miscasedAdminPathProvider(): array
    {
        return [
            '컨트롤러 전부 대문자' => ['/admin/MEMBER/delete/5'],
            '컨트롤러 섞임'        => ['/admin/MeMbEr/delete/5'],
            '메서드 대문자'        => ['/admin/member/DELETE/5'],
            '메서드 섞임'          => ['/admin/member/DeLeTe/5'],
        ];
    }

    public function testCanonicalAdminPathStillResolves(): void
    {
        $result = $this->resolve('/admin/member/delete/5', true, 'POST');

        $this->assertSame(\Mublo\Controller\Admin\MemberController::class, $result['controller']);
        $this->assertSame('delete', $result['method']);
        $this->assertSame(['5'], $result['params']);
    }

    /**
     * kebab-case 는 정상 표기다 — 이걸 막으면 관리자 화면이 깨진다.
     */
    public function testKebabCasePathStillResolves(): void
    {
        $result = $this->resolve('/admin/block-row/get-content-items', true, 'POST');

        $this->assertSame('getContentItems', $result['method']);
    }
}
