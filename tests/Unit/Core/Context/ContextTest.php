...
class ContextTest extends TestCase
{
    ...
    public function testContextAcceptsStableDomainDescriptor(): void
    {
        $context = new Context($this->request);
        $context->setDomainDescriptor(new DomainDescriptor(
            domainId: 7,
            ...
            status: 'active',
            siteTitle: 'Rental',
            companyConfig: ['name' => 'Mublo'],
            seoConfig: ['title' => 'Rental SEO'],
        ));

        $domain = $context->getDomainInfo();

        $this->assertSame('rental.example.com', $context->getDomain());
        $this->assertSame(7, $context->getDomainId());
        $this->assertSame('1/7', $context->getDomainGroup());
        $this->assertSame('Rental', $domain?->getSiteTitle());
        $this->assertSame(['name' => 'Mublo'], $domain?->getCompanyConfig());
        $this->assertSame(['title' => 'Rental SEO'], $domain?->getSeoConfig());
    }
     ...
     * SUPER 도메인 판정 테스트
     */
    public function testContextSuperDomain(): void
    {
        $this->assertTrue($this->context->isSuperDomain());

        $childContext = new Context($this->request);
        $childContext->setDomainInfo(new Domain(
            domainId: 2,
            domain: 'child.example.com',
            domainGroup: '1/2'
        ));
        $this->assertFalse($childContext->isSuperDomain());

        $contextWithoutDomain = new Context($this->request);
        $this->assertFalse($contextWithoutDomain->isSuperDomain());
    }
