...
window.MubloItemLayout = (function () {
    ...
    // 관리자 설정(BlockColumn::normalizeAutoplay)과 같은 상한. 읽는 쪽에서
    // 클램프하므로 렌더러·직접입력 HTML 어느 채널로 와도 동작이 일치한다.
    const MAX_AUTOPLAY_MS = 30000;
    ...
    function init(el, options) {
        ...
        const pcAutoplay = Math.min(MAX_AUTOPLAY_MS, Math.max(0, parseInt(el.dataset.pcAutoplay, 10) || 0));
        const moAutoplay = Math.min(MAX_AUTOPLAY_MS, Math.max(0, parseInt(el.dataset.moAutoplay, 10) || 0));
