...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        /**
         * 스택 렌더가 바꿔 놓은 칸 정보(배지 = "콘텐츠 N개", 버튼 = [칸 스타일])를
         * 단일 칸 표기로 되돌린다 — 스택 → 단일 축소 뒤에도 카드가 원래 모습
         * (미설정/타입 배지 + [설정]·[수정])으로 보이게 한다.
         *
         * @return {boolean} 이 칸에 콘텐츠가 설정되어 있는지
         */
        function restoreSingleCardInfo(card, i) {
            const legacy = BlockContentStack.readLegacy(i);
            const hasContent = (legacy.content_type || '') !== '';

            const badge = card.querySelector('.column-type-badge');
            if (badge) {
                badge.textContent = hasContent ? typeLabel(legacy.content_type) : '미설정';
                badge.classList.toggle('bg-primary', hasContent);
                badge.classList.toggle('bg-secondary', !hasContent);
            }

            // 칸 설정 버튼 = 콘텐츠 목록(.stack-content-list) 바깥의 버튼
            const infoButton = Array.from(card.querySelectorAll('.card-body button'))
                .find((button) => !button.closest('.stack-content-list'));
            if (infoButton) {
                infoButton.innerHTML = hasContent ? '수정' : '설정';
                infoButton.title = '';
            }
        ...
        function renderCard(card) {
            ...
            if (!BlockContentStack.isStack(i)) {
                ...
                // 콘텐츠가 없는 칸에는 [콘텐츠 추가]를 두지 않는다: 첫 콘텐츠는
                // [설정] 모달이 만들고, "추가"는 두 번째 콘텐츠부터라야 말이 된다.
                const hasContent = restoreSingleCardInfo(card, i);
        ...
        /**
         * claimContentFileInputs 의 역방향 — 스택 → 단일 축소 시 유지할 콘텐츠의
         * 미저장 파일 인풋을 칸 채널로 되돌리고 나머지는 버린다. 단일 칸은
         * 서버가 칸 채널만 읽으므로 옮기지 않으면 업로드가 조용히 유실된다.
         */
        function releaseContentFileInputs(i, keepJ) {
            const form = fileInputRoot();
            if (!form) return;
            FILE_CHANNELS.forEach(([columnChannel, contentChannel]) => {
                // 남아 있던 칸 채널 인풋은 유지할 콘텐츠의 것으로 대체
                form.querySelectorAll(`input[type="file"][name^="${columnChannel}[${i}]["]`)
                    .forEach((el) => el.remove());
                const keepPrefix = `${contentChannel}[${i}][${keepJ}]`;
                form.querySelectorAll(`input[type="file"][name^="${contentChannel}[${i}]["]`).forEach((el) => {
                    if (!el.name.startsWith(keepPrefix)) { el.remove(); return; }
                    el.name = `${columnChannel}[${i}]${el.name.slice(keepPrefix.length)}`;
                });
            });
        }
        ...
        preview.addEventListener('click', (event) => {
            ...
            switch (button.dataset.stackAct) {
                ...
                case 'del': {
                    const drafts = BlockContentStack.readContents(i);
                    if (!drafts[j]) break;
                    const label = typeLabel(drafts[j].content_type);

                    // 마지막 1개 삭제 = 칸 비우기 → 콘텐츠 없는 단일 칸으로 복귀
                    if (drafts.length <= 1) {
                        if (!confirm(`콘텐츠(${label})를 삭제하면 이 칸은 콘텐츠가 없는 상태가 됩니다. 삭제하시겠습니까?`)) break;
                        BlockContentStack.reduceToSingle(i, j);
                        BlockContentStack.writeLegacy(i, { content_kind: 'CORE' });
                        remapContentFileInputs(i, () => null);
                        renderAll();
                        break;
                    }

                    if (!confirm(`콘텐츠(${label})를 삭제하시겠습니까?`)) break;
                    BlockContentStack.removeContent(i, j);
                    remapContentFileInputs(i, (x) => (x === j ? null : (x > j ? x - 1 : x)));
                    // 1개만 남으면 다시 단일 칸으로 축소 (계획 5.3 명시적 축소) —
                    // 단일 ↔ 다중 전이를 대칭으로 만들어 되돌릴 길을 남긴다.
                    if (BlockContentStack.readContents(i).length === 1) {
                        BlockContentStack.reduceToSingle(i, 0);
                        releaseContentFileInputs(i, 0);
                    }
                    renderAll();
                    break;
                }
