...
(function() {
    ...
    class BlockRowForm {
        ...
        updateSkinList(contentType) {
            ...
            if (noSkinTypes.includes(contentType)) {
                // 스킨 선택 숨김 (html/include 등 스킨이 없는 타입)
                if (skinWrapper) skinWrapper.style.display = 'none';
                skinSelect.innerHTML = '<option value="">스킨 없음</option>';
                return;
            }

            // 미선택(선택하세요) 등 그 외 타입: 스킨 래퍼 유지
            // (toggleHtmlEditor의 표시 처리와 일관 — 레이아웃이 접히지 않도록)
        ...
        toggleHtmlEditor(contentType) {
            ...
            if (contentType === 'html') {
                ...
                this.initBlockHtmlLandingEditor();
                ...
                // menu 타입: 갯수 숨김, 스킨 + 전용 설정(범위/방향/깊이) 표시
                if (countWrapper) countWrapper.style.display = 'none';
                if (countMoWrapper) countMoWrapper.style.display = 'none';
                if (skinWrapper) skinWrapper.style.display = '';
                if (menuConfigWrapper) menuConfigWrapper.style.display = 'block';
            } else {
                // 다른 타입: 갯수/스킨 표시
        ...
        /**
         * 파일 용량 표시 유틸 — 선택한 파일 크기를 라벨에 쓰고, 제출 한도(MubloRequest
         * .maxFileSize, 기본 10MB) 초과 시 빨간색으로 강조한다. 서버로 나가기 전에 어떤
         * 이미지가 용량 초과인지 그 자리에서 알려주기 위한 것.
         */
        getMaxFileSize() {
            try { return MubloRequest.getConfig().maxFileSize || (10 * 1024 * 1024); }
            catch (e) { return 10 * 1024 * 1024; }
        }
        ...
        setFileSizeLabel(labelEl, sizeBytes) {
            ...
            const over = sizeBytes > this.getMaxFileSize();
            labelEl.textContent = this.formatFileSize(sizeBytes)
                + (over ? ` · 최대 ${this.formatFileSize(this.getMaxFileSize())} 초과` : '');
            labelEl.classList.add(over ? 'text-danger' : 'text-muted');
            if (over) labelEl.classList.add('fw-bold');
        }
        ...
        openColumnModal(index) {
            ...
            setTimeout(() => {
                const editorInstance = this.blockHtmlVisualEditor
                    || (typeof BlockHtmlEditor !== 'undefined' ? BlockHtmlEditor.getVisual?.('modal_html_content') : null);
                if (editorInstance) {
            ...
            // menu 설정 로드 (렌더러 기본값: all / horizontal / 2)
            const menuScope = document.getElementById('modal_menu_scope');
            const menuOrientation = document.getElementById('modal_menu_orientation');
            const menuDepth = document.getElementById('modal_menu_depth');
            if (menuScope) menuScope.value = contentConfig.scope === 'current' ? 'current' : 'all';
            if (menuOrientation) menuOrientation.value = contentConfig.orientation === 'vertical' ? 'vertical' : 'horizontal';
            if (menuDepth) menuDepth.value = String(parseInt(contentConfig.depth, 10) || 2);
            ...
            if (videoTypeSelect) {
                // 렌더러 측 'type' 키와 'video' 값도 호환 (PHP가 'video'로 저장한 경우 select의 'url' 옵션으로 매핑)
                let savedType = contentConfig.video_type || contentConfig.type || 'youtube';
                if (savedType === 'video') savedType = 'url';
                videoTypeSelect.value = savedType;
            }
        ...
        saveColumnSettings() {
            ...
            if (contentType === 'html') {
                ...
                contentConfig.scope = document.getElementById('modal_menu_scope')?.value === 'current' ? 'current' : 'all';
                contentConfig.orientation = document.getElementById('modal_menu_orientation')?.value === 'vertical' ? 'vertical' : 'horizontal';
                contentConfig.depth = parseInt(document.getElementById('modal_menu_depth')?.value, 10) || 2;
            } else if (contentType === 'movie') {
                ...
                // 렌더러가 읽는 'type' 키도 동기화 ('url'은 HTML5 video로 매핑)
                contentConfig.type = videoType === 'url' ? 'video' : videoType;
    ...
    class DualListbox {
        ...
        renderItems(list, items) {
            list.replaceChildren(...items.map(item => {
                ...
                element.className = 'dual-listbox-item';
