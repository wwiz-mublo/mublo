 ...
 *
 * 서버 데이터는 이 파일에 박지 않는다. 뷰(views/Admin/Blockeditor/Index.php)가
 * window.MubloBlockEditorConfig 로 넘기고 여기서는 읽기만 한다. 앞의
 * MubloFrontPreviewTokens 와 같은 방식이다.
 *
 * 그래서 이 파일은 PHP 가 섞이지 않은 정적 자산이다 — 브라우저·CDN 이 캐시하고,
 * 린터와 정적 분석이 읽을 수 있고, 스택 트레이스의 줄 번호가 실제 줄과 맞는다.
 * 인라인이던 시절엔 셋 다 불가능했다.
 *
 * 이 스크립트는 DOM 을 즉시 조회하므로(els) 반드시 마크업 뒤에서,
 * 그리고 의존 전역(MubloRequest·BlockHtmlEditor·MubloBlockCapabilities)을
 * 싣는 스크립트 뒤에서 동기 로드해야 한다. defer/async 를 붙이면 순서가 깨진다.
 * ===================================================================== */
(function () {
    'use strict';

    // 뷰가 심어 둔 초기 데이터. 폴백은 각 값의 실제 사용 형태에 맞춘다
    // (CONTENT_TYPES 는 .find()/.forEach() 를 쓰므로 배열, 나머지 둘은 맵).
    const CONFIG = window.MubloBlockEditorConfig || {};

    const CONTEXTS = CONFIG.contexts || {};
    const INITIAL = CONFIG.initialContext || '';
    ...
    const CONTENT_TYPES = CONFIG.contentTypes || [];
    const SKIN_LISTS = CONFIG.skinLists || {};
    const CAN_EDIT_INCLUDE = CONFIG.canEditInclude === true;
    ...
    els.overlay.addEventListener('click', (e) => {
        ...
        if (t && t.kind === 'column') {
            ...
            if (usesModalEditor(col.content_type)) {
    ...
    function openColumnByIndex(rowId, colIndex) {
        ...
        if (target) {
            ...
            } else if (colMeta && usesModalEditor(colMeta.content_type)) {
    ...
    function outputStyleHtml(c) {
        ...
        const device = (dev, label, defCols, defDelay) => {
            ...
            return `<section class="bke-output-device">
                <div class="bke-output-device__title">${label}</div>
                <div class="bke-form-row">
        ...
        return `<div class="bke-output-devices">
    ...
    /* 미저장 이탈 방지 — 이 편집기는 '초안 저장'을 눌러야 서버에 남는다.
       열 때 기준선을 찍어두고, 닫기·새로고침 전에 달라졌는지 비교한다. */
    function bkefSignature() {
        return JSON.stringify([bkef.html.value, bkef.css.value, bkef.js.value]);
    }
    /** 현재 내용을 '저장된 상태'로 삼는다 (열기 직후 · 초안 저장 성공 후). */
    function bkefMarkClean() { frameState.baseline = bkefSignature(); }
    function bkefIsDirty() {
        return bkef.modal.classList.contains('open')
            && frameState.baseline != null
            && bkefSignature() !== frameState.baseline;
    }
    /** 모달을 닫는다 — 기준선을 지워 닫은 뒤 이탈 경고가 뜨지 않게 한다. */
    function bkefClose() {
        frameState.baseline = null;
    ...
    document.querySelectorAll('[data-bkef-close]').forEach(el =>
        el.addEventListener('click', () => {
            if (!bkefIsDirty()) { bkefClose(); return; }
            MubloRequest.showConfirm(
                '저장하지 않은 편집 내용이 있습니다. 닫으면 사라집니다.',
                bkefClose,
                { type: 'warning', confirmText: '닫기', cancelText: '계속 편집' }
            );
        }));
    ...
    async function openFrameEditor(part, statusData) {
        ...
        bkefMarkClean();      // 여기까지가 '저장된 상태' — 이후 변경분만 미저장으로 본다
    }
    ...
    async function frameSaveDraft(silent) {
        ...
        try {
            ...
            bkefMarkClean(); // 저장됐으니 기준선을 현재 내용으로 옮긴다
    ...
    function usesModalEditor(type) {
        return typeInfo(type)?.editor?.mode === 'modal';
    }

    // 타입별 대표 아이콘 — 등록부가 선언한 값을 쓴다. 코어는 확장 타입의
    // 아이콘을 추측하지 않는다(BlockRegistry options['icon']).
    function typeIcon(type) {
        return typeInfo(type)?.icon || 'bi-box';
    }
    ...
    async function renderColumnEditor(target, meta, backHtml) {
        ...
        // 이 화면에서 스택을 해제(마지막 1개까지 삭제)했는지 — 적용 시 서버에
        // content_mode=single 을 명시해야 하위 콘텐츠가 정리된다 (계획 5.3)
        ...
        /** 임의의 콘텐츠 소스(칸·스택 draft·빈 객체)를 편집 상태로 적재 */
        const loadContentFrom = (src) => {
        ...
        const loadContentState = () => loadContentFrom(contentSource());
        loadContentState();

        // 전용 선택 UI(업로드·커스텀 셀렉터) 타입은 네이티브 피커로 다루지 않는다.
        const isComplexItems = () => MubloBlockCapabilities.forType(typeInfo(cs.type)).customConfig
            || usesModalEditor(cs.type)
            || cs.items.some(v => v !== null && typeof v === 'object');

        const paint = () => {
            const info = typeInfo(cs.type);
            const capabilities = MubloBlockCapabilities.forType(info);
            const hasItems = capabilities.items;
            const modalEditor = usesModalEditor(cs.type);
            const outputSettings = MubloBlockCapabilities.outputSettings(info, modalEditor);
            ...
            if (cs.type && capabilities.skin && skins.length) {
                html += `<details class="bke-acc">
                    <summary><i class="bi bi-brush"></i> 디자인 (스킨) <i class="bi bi-chevron-down"></i></summary>
                    <div class="bke-acc__body">
                        <div class="bke-form-row" style="margin-bottom:0;">
                            <label class="bke-form-label" for="cf_skin">스킨 선택</label>
                            <select class="form-select form-select-sm" id="cf_skin" aria-label="디자인 스킨">
            ...
            // Registry capability가 선언한 공통 출력 설정만 표시한다. modal 편집기가
            // 자체 제공하는 출력 스타일은 중복하지 않되 AOS/count 같은 공통 설정은
            // 편집 방식과 무관하게 칸 패널에서 제공한다.
            const hasPanelOutputSettings = outputSettings.count || outputSettings.style || outputSettings.aos;
            if (cs.type && hasPanelOutputSettings) {
                const cc = cs.config;
                html += `<details class="bke-acc">
                    <summary><i class="bi bi-grid-3x3-gap"></i> 출력 설정 <i class="bi bi-chevron-down"></i></summary>
                    <div class="bke-acc__body" id="cfOutputStyle">
                        ${outputSettings.count ? `<div class="bke-form-grid">
                            <div class="bke-form-row">
                        ...
                        ${outputSettings.style ? outputStyleHtml(cc) : ''}
                        ${outputSettings.aos ? `<div class="bke-form-grid" style="margin-top:8px;">
                            <div class="bke-form-row">
                                <label class="bke-form-label">출력 이벤트</label>
                                <select class="form-select form-select-sm" id="cf_aos">
                                    ${['|없음','fade-up|Fade Up','fade-down|Fade Down','fade-left|Fade Left','fade-right|Fade Right','zoom-in|Zoom In','zoom-out|Zoom Out'].map(option => {
                                        const [value, label] = option.split('|');
                                        return `<option value="${value}" ${String(cc.aos || '') === value ? 'selected' : ''}>${label}</option>`;
                                    }).join('')}
                                </select>
                            </div>
                            <div class="bke-form-row">
                                <label class="bke-form-label">이벤트 시간(ms)</label>
                                <input type="number" class="form-control form-control-sm" id="cf_aos_duration" min="100" max="3000" step="100" value="${parseInt(cc.aos_duration) || 600}">
                            </div>
                        </div>` : ''}
                    </div>
                </details>`;
            ...
            els.inspector.querySelectorAll('[data-cfstk-del]').forEach((btn) => {
                btn.addEventListener('click', () => {
                    ...
                    // 마지막 1개 삭제 = 칸 비우기 (단일 칸으로 복귀)
                    const isLast = stackContents.length <= 1;
                    const message = isLast
                        ? `콘텐츠(${delLabel})를 삭제하면 이 칸은 콘텐츠가 없는 상태가 됩니다. [적용]을 눌러야 저장됩니다.`
                        : `콘텐츠(${delLabel})를 삭제하시겠습니까? [적용]을 눌러야 저장됩니다.`;
                    MubloRequest.showConfirm(message, () => {
                        if (!isLast) harvestContent(); // 다른 콘텐츠 삭제 시 현재 편집값 보존
                        stackContents.splice(j, 1);
                        if (j < stackIndex) stackIndex -= 1;
                        if (stackIndex >= stackContents.length) stackIndex = stackContents.length - 1;
                        // 1개 이하로 줄면 단일 칸으로 축소 — 단일 ↔ 다중 전이를
                        // 대칭으로 만들어 되돌릴 길을 남긴다 (계획 5.3 명시적 축소)
                        if (stackContents.length <= 1) {
                            const keep = stackContents[0] || {};
                            isStackCol = false;
                            stackContents = null;
                            stackIndex = null;
                            stackReduced = true;
                            loadContentFrom(keep);
                        } else {
                            loadContentState();
                        }
                        paint();
            ...
            document.getElementById('cfAddStackContent')?.addEventListener('click', () => {
                // 단일 → 스택 전환 (draft): 편집 중인 콘텐츠(미적용 값 포함)를 첫
                // 항목으로 옮기고, 새 항목을 추가해 바로 편집한다
                isStackCol = true;
                stackReduced = false;
                stackContents = [{ ...collectContentDraft(), is_active: 1 }];
                stackContents.push({ content_type: '', content_kind: 'CORE', content_skin: '', title_config: '{}', content_config: '{}', content_items: '[]', is_active: 1 });
                stackIndex = stackContents.length - 1;
                loadContentState();
                paint();
        ...
        const stackState = () => ({ isStackCol, stackContents, stackIndex, reduced: stackReduced });

        /**
         * 지금 편집 화면에 떠 있는 값을 콘텐츠 draft 한 벌로 걷어온다
         * (입력 요소가 없으면 cs 상태만 반영).
         */
        const collectContentDraft = () => {
            ...
            const aos = document.getElementById('cf_aos');
            const aosDuration = document.getElementById('cf_aos_duration');
            if (aos) config.aos = aos.value || null;
            if (aosDuration) config.aos_duration = parseInt(aosDuration.value) || 600;
        ...
        const harvestContent = () => {
            ...
            stackContents[stackIndex] = { ...stackContents[stackIndex], ...collectContentDraft() };
        };
    ...
    async function applyColumnForm(target, data, colIndex, cs, stack = null) {
        ...
        if (isStack) {
            ...
            if (stack && stack.reduced) {
                // 스택 해제 — 유지할 콘텐츠는 위에서 칸의 레거시 필드로 들어갔다.
                // content_mode=single 을 명시해야 서버가 하위 콘텐츠를 정리한다 (계획 5.3).
                c.content_mode = 'single';
                delete c.contents;
            }
        }
    ...
    async function openContentModal(target, meta, draftType, opts = {}) {
        ...
        cm = {
            ...
            adapter: typeInfo(draftType || src.content_type)?.editor?.adapter || '',
            config: typeChanged ? {} : parse(src.content_config, {}),
            items: typeChanged ? [] : parseList(src.content_items),
        };

        const adapterLabels = { html: 'HTML', image: '이미지', movie: '동영상' };
        const cmPos = stackIndex !== null
            ? `› ${colIndex + 1}번째 칸 · ${stackIndex + 1}번째 콘텐츠 · ${adapterLabels[cm.adapter] || ''} 내용`
            : `› ${colIndex + 1}번째 칸 · ${adapterLabels[cm.adapter] || ''} 내용`;
        ...
        cmEls.html.style.display = cm.adapter === 'html' ? '' : 'none';
        cmEls.image.style.display = cm.adapter === 'image' ? '' : 'none';
        cmEls.video.style.display = cm.adapter === 'movie' ? '' : 'none';
        cmEls.body.classList.toggle('bke-contentmodal__body--fill', cm.adapter === 'html');
        ...
        if (cm.adapter === 'html') {
            ...
            setTimeout(() => {
                ...
                cmMarkClean(); // 편집기가 붙어 내용이 정규화된 뒤에 기준선을 찍는다
            }, 60);
        } else if (cm.adapter === 'image') {
            if (!cm.items.length) cm.items = [{ pc_image: '', mo_image: '', link_url: '', link_win: '0', title: '', desc: '' }];
    ...
    /* 미저장 이탈 방지 — 이 모달은 '저장'을 눌러야 서버에 반영된다.
       배경 클릭 한 번으로 편집분이 사라지던 자리다. */
    function cmSignature() {
        if (!cm) return '';
        if (cm.adapter === 'html') {
            // 저장 경로(bkecDoSaveContent)와 같은 방식으로 현재 내용을 읽는다
            const editor = cm.editor || BlockHtmlEditor.getVisual('bke_html_content');
            let html;
            if (editor) { editor.sync?.(); html = editor.getHTML(); }
            else { html = document.getElementById('bke_html_content').value; }
            return JSON.stringify([html,
                document.getElementById('bke_html_css').value,
                document.getElementById('bke_html_js').value]);
        }
        if (cm.adapter === 'image') {
            // File 객체는 직렬화되지 않는다 — 이름·크기로 대신 비교한다
            return JSON.stringify([cm.config, cm.items.map(({ _pcFile, _moFile, _pcPreview, ...rest }) => ({
                ...rest,
                _pc: _pcFile ? _pcFile.name + ':' + _pcFile.size : '',
                _mo: _moFile ? _moFile.name + ':' + _moFile.size : '',
            }))]);
        }
        if (cm.adapter === 'movie') {
            return JSON.stringify([
                document.getElementById('bkec_video_type').value,
                document.getElementById('bkec_video_url').value,
                document.getElementById('bkec_video_autoplay').checked,
                document.getElementById('bkec_video_muted').checked,
            ]);
        }
        return '';
    }
    /** 현재 내용을 '저장된 상태'로 삼는다 (편집기 부착 직후). */
    function cmMarkClean() { if (cm) cm.baseline = cmSignature(); }
    function cmIsDirty() { return !!cm && cm.baseline != null && cmSignature() !== cm.baseline; }

    cmEls.modal.querySelectorAll('[data-bkec-close]').forEach(el =>
        el.addEventListener('click', () => {
            if (!cmIsDirty()) { closeContentModal(); return; }
            MubloRequest.showConfirm(
                '저장하지 않은 편집 내용이 있습니다. 닫으면 사라집니다.',
                closeContentModal,
                { type: 'warning', confirmText: '닫기', cancelText: '계속 편집' }
    ...
    /* 탭 닫기·새로고침·뒤로가기 — 두 편집 모달의 미저장 내용을 브라우저 경고로 막는다.
       (문구는 브라우저가 정한다. preventDefault 와 returnValue 를 각각 보는 브라우저가 있어 둘 다 쓴다.) */
    window.addEventListener('beforeunload', (e) => {
        if (!bkefIsDirty() && !cmIsDirty()) return;
        e.preventDefault();
        e.returnValue = '';
    });
    ...
    async function bkecRunQuality(serverAudit) {
        ...
        if (!panel || !cm || cm.adapter !== 'html') return;
        const editor = cm.editor || BlockHtmlEditor.getVisual('bke_html_content');
        editor?.sync?.();
    ...
    document.getElementById('bkecAiGenerate').addEventListener('click', async () => {
        if (!cm || cm.adapter !== 'html' || aiAbort) return;
    ...
    window.addEventListener('focus', () => {
        if (cm?.adapter === 'html' && !aiAbort) loadAiLibrary();
    });
    ...
    function renderImageItemsEditor() {
        ...
        html += '<div style="font-size:12px;color:var(--bs-secondary-color);margin-bottom:10px;">'
            + '여러 장을 등록하면 위 출력 스타일대로 함께 표시됩니다. 여백·테두리는 칸 편집의 [스타일]에서.<br>'
            + '제목·설명은 선택 사항입니다 — 비워 두면 이미지만 나옵니다.</div>';
        ...
        cm.items.forEach((item, i) => {
            ...
            html += `<div class="bke-imgitem" data-idx="${i}">
                ...
                <div class="bke-imgitem__fields">
                    ...
                    <div class="bke-imgitem__row">
                        <label class="bke-form-label">제목 <span style="font-weight:400;">(선택)</span></label>
                        <input type="text" class="form-control form-control-sm" data-img="title" placeholder="이미지 아래에 표시할 한 줄" value="${esc(item.title || '')}">
                    </div>
                    <div class="bke-imgitem__row">
                        <label class="bke-form-label">설명 <span style="font-weight:400;">(선택)</span></label>
                        <input type="text" class="form-control form-control-sm" data-img="desc" placeholder="제목 아래 보조 설명" value="${esc(item.desc || '')}">
                    </div>
                </div>
        ...
        cmEls.image.querySelectorAll('.bke-imgitem').forEach(row => {
            ...
            row.querySelector('[data-img="title"]').addEventListener('input', (e) => { item.title = e.target.value; });
            row.querySelector('[data-img="desc"]').addEventListener('input', (e) => { item.desc = e.target.value; });
        ...
        document.getElementById('bkecAddImage').addEventListener('click', () => {
            cm.items.push({ pc_image: '', mo_image: '', link_url: '', link_win: '0', title: '', desc: '' });
            renderImageItemsEditor();
    ...
    cmEls.save.addEventListener('click', () => {
        ...
        if (cm.adapter === 'html' && document.getElementById('bkecQuality').dataset.status === 'needs_fix') {
            MubloRequest.showConfirm(
